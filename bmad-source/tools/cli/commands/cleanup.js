...
module.exports = {
	...
	options: [
		...
		[
			"-a, --auto-delete",
			"Automatically delete non-retained files without prompts",
		],
	...
	action: async (options) => {
		try {
			...
			const retentionPath = nodePath.join(
				bmadDir,
				"_cfg",
			...
			if (options.listRetained) {
				...
				if (await fs.pathExists(retentionPath)) {
					...
					const retentionData = yaml.load(retentionContent) || {
						retainedFiles: [],
					};
			...
			if (options.dryRun) {
				...
				console.log(
					chalk.cyan(
						`Found ${allLegacyFiles.length} legacy file(s) that could be cleaned up.\n`,
					),
				);
				console.log(
					chalk.dim('Run "bmad cleanup" to actually delete these files.\n'),
				);
			...
			const result = await installer.performCleanup(
				bmadDir,
				options.autoDelete,
			);
			...
			if (result.message) {
				...
				if (result.deleted > 0) {
					console.log(
						chalk.green(`✅ Deleted ${result.deleted} legacy file(s)`),
					);
				}
				if (result.retained > 0) {
					...
					console.log(
						chalk.dim(
							'Run "bmad cleanup --list-retained" to see retained files\n',
						),
					);
				}
			}
