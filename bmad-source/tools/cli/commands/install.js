	...
	action: async (options) => {
		try {
			...
			if (config.actionType === "compile") {
				...
				console.log(
					chalk.cyan(
						`Rebuilt ${result.agentCount} agents and ${result.taskCount} tasks`,
					),
				);
				process.exit(0);
				return;
			...
			if (config.actionType === "quick-update") {
				...
				console.log(
					chalk.cyan(
						`Updated ${result.moduleCount} modules with preserved settings`,
					),
				);
				process.exit(0);
				return;
			...
			if (result?.success) {
				...
				if (result.needsAgentVibes) {
					...
					await new Promise((resolve) => {
						rl.question(
							chalk.green("Press Enter to start AgentVibes installer..."),
							() => {
								rl.close();
								resolve();
							},
