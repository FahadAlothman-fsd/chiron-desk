...
module.exports = {
	...
	options: [
		...
		[
			"-t, --destination <path>",
			"Target installation directory (default: current project BMAD installation)",
		],
	],
	action: async (options) => {
		try {
			...
			if (!config) {
				console.log(
					chalk.yellow("No BMAD installation found in current directory."),
				);
			...
			if (options.source) {
				...
				if (stat.isFile() && providedPath.endsWith(".agent.yaml")) {
					...
					if (yamlFiles.length === 1) {
						...
						console.log(
							chalk.red("Directory must contain exactly one .agent.yaml file"),
						);
						process.exit(1);
					}
				} else {
					console.log(
						chalk.red(
							"Path must be an .agent.yaml file or a folder containing one",
						),
					);
					process.exit(1);
				}
				...
				console.log(
					chalk.dim(`Searching for agents in: ${customAgentLocation}\n`),
				);
				...
				if (agents.length === 0) {
					console.log(
						chalk.yellow("No agents found in custom agent location."),
					);
				...
				for (const [idx, agent] of agents.entries()) {
					...
					console.log(
						`  ${idx + 1}. ${typeIcon} ${chalk.bold(agent.name)} ${chalk.dim(`(${agent.type})`)}`,
					);
				}
				...
				const selectedIdx = Number.parseInt(selection, 10) - 1;
				if (
					Number.isNaN(selectedIdx) ||
					selectedIdx < 0 ||
					selectedIdx >= agents.length
				) {
			...
			console.log(
				chalk.dim("   Leave blank to use default, or provide a custom name."),
			);
			console.log(chalk.dim("   Examples:"));
			console.log(
				chalk.dim(
					`     - (blank) → "${defaultPersonaName}" as ${agentType}.md`,
				),
			);
			console.log(chalk.dim(`     - "Fred" → "Fred" as fred-${agentType}.md`));
			console.log(
				chalk.dim(
					`     - "Captain Code" → "Captain Code" as captain-code-${agentType}.md`,
				),
			);
			...
			if (agentConfig.installConfig && !options.defaults) {
				answers = await promptInstallQuestions(
					agentConfig.installConfig,
			...
			let targetDir = options.destination
			...
			if (targetDir) {
				...
				if (otherProject && !targetDir.includes("agents")) {
					...
					console.log(
						chalk.dim(`   Auto-selecting custom agents folder: ${targetDir}`),
					);
				}
			} else {
				const rl = readline.createInterface({
				...
				if (choice.trim() === "1" || choice.trim() === "") {
					...
					if (otherProject) {
						targetDir = path.join(otherProject.bmadFolder, "custom", "agents");
						console.log(
							chalk.dim(`   Found BMAD project, using: ${targetDir}`),
						);
					} else {
			...
			if (targetProject) {
				console.log(
					chalk.cyan(
						`   Detected BMAD project at: ${targetProject.projectRoot}`,
					),
				);
			}
			...
			if (targetProject) {
				...
				existingEntry = checkManifestForPath(
					targetProject.manifestFile,
					expectedPath,
				);
			...
			const projectRoot = targetProject
			...
			if (selectedAgent.hasSidecar && selectedAgent.type === "expert") {
				...
				const sidecarFiles = copySidecarFiles(
					selectedAgent.path,
					agentTargetDir,
					selectedAgent.yamlFile,
				);
			...
			console.log(
				chalk.cyan(`   Compiled: ${path.basename(result.compiledFile)}`),
			);

			if (result.sidecarCopied) {
				console.log(
					chalk.cyan(
						`   Sidecar files: ${result.sidecarFiles.length} files copied`,
					),
				);
			}
			...
			if (targetProject) {
				...
				console.log(
					chalk.dim(`Registering in manifest: ${targetProject.manifestFile}`),
				);
				...
				if (shouldUpdateExisting && existingEntry) {
					updateManifestEntry(
						targetProject.manifestFile,
						manifestData,
						existingEntry._lineNumber,
					);
					console.log(
						chalk.green("   ✓ Updated existing entry in agent-manifest.csv"),
					);
				} else {
				...
				const ideResults = await createIdeSlashCommands(
					targetProject.projectRoot,
					finalAgentName,
					relativePath,
					metadata,
				...
				const manifestYamlPath = path.join(
					targetProject.cfgFolder,
					"manifest.yaml",
				);
