	...
	action: async (agentName, options) => {
		try {
			...
			if (!claudeDir) {
				...
				console.log(
					chalk.dim("Run this command from your project directory or"),
				);
...
async function buildAgent(projectDir, agentName, force = false) {
	...
	let standaloneYamlPath = path.join(
		standaloneAgentDir,
	...
	if (
		!(await fs.pathExists(standaloneYamlPath)) &&
		(await fs.pathExists(standaloneAgentDir))
	) {
	...
	if (await fs.pathExists(standaloneYamlPath)) {
		...
		console.log(
			chalk.green(`  ✓ ${agentName} built successfully (standalone)`),
		);
		return;
	}
...
async function checkBuildStatus(projectDir) {
	...
	if (needsRebuild.length === 0) {
		...
		console.log(
			chalk.yellow(`${needsRebuild.length} agent(s) need rebuilding:`),
		);
 ...
 */
async function checkIfNeedsRebuild(
	yamlPath,
	outputPath,
	projectDir,
	agentName,
	...
	if (await fs.pathExists(customizePath)) {
		const customizeMetaMatch = outputContent.match(
			/customize:.*\(hash: ([a-f0-9]+)\)/,
		);
