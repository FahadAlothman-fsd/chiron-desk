...
class RovoDevSetup extends BaseIdeSetup {
	...
	async cleanup(projectDir) {
		...
		if (await fs.pathExists(subagentsDir)) {
			...
			const bmadFiles = entries.filter(
				(file) => file.startsWith("bmad-") && file.endsWith(".md"),
			);
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		for (const artifact of agentArtifacts) {
			...
			const subagentContent = this.convertToRovoDevSubagent(
				artifact.content,
				artifact.name,
				artifact.module,
			);
		...
		const { tasks: taskCount, tools: toolCount } =
			await this.generateTaskToolReferences(
				bmadDir,
				referencesDir,
				taskToolGen,
			);
		...
		console.log(
			chalk.dim(`  - ${agentCount} agents installed to .rovodev/subagents/`),
		);
		if (workflowCount > 0) {
			console.log(
				chalk.dim(
					`  - ${workflowCount} workflows installed to .rovodev/workflows/`,
				),
			);
		}
		if (taskCount + toolCount > 0) {
			console.log(
				chalk.dim(
					`  - ${taskCount + toolCount} tasks/tools installed to .rovodev/references/ (${taskCount} tasks, ${toolCount} tools)`,
				),
			);
		}
		console.log(
		...
		console.log(
			chalk.dim(
				"  - Access agents by typing @ in Rovo Dev to see available options",
			),
		);
		console.log(
			chalk.dim(
				"  - Workflows and references are available in .rovodev/ directory",
			),
		);
	...
	async generateTaskToolReferences(bmadDir, referencesDir, taskToolGen) {
		...
		const standaloneTasks = tasks
			? tasks.filter((t) => t.standalone === "true" || t.standalone === true)
			: [];
		const standaloneTools = tools
			? tools.filter((t) => t.standalone === "true" || t.standalone === true)
			: [];
		...
		for (const task of standaloneTasks) {
			...
			const targetPath = path.join(
				referencesDir,
				`bmad-task-${task.module}-${task.name}.md`,
			);
		...
		for (const tool of standaloneTools) {
			...
			const targetPath = path.join(
				referencesDir,
				`bmad-tool-${tool.module}-${tool.name}.md`,
			);
			await this.writeFile(targetPath, commandContent);
			toolCount++;
		}
	...
	convertToRovoDevSubagent(launcherContent, agentName, moduleName) {
		...
		const roleDefinitionMatch = launcherContent.match(
			/roleDefinition="([^"]+)"/,
		);
		const roleDefinition = roleDefinitionMatch
			? roleDefinitionMatch[1]
			: `You are a specialized agent for ${title.toLowerCase()} tasks.`;
		...
		const instructionsMatch = launcherContent.match(
	...
	async detect(projectDir) {
		...
		if (await fs.pathExists(subagentsDir)) {
			try {
				...
				if (
					entries.some(
						(entry) => entry.startsWith("bmad-") && entry.endsWith(".md"),
					)
				) {
					return true;
				}
