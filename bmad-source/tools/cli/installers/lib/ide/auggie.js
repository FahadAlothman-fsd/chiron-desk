	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		for (const artifact of agentArtifacts) {
			const targetPath = path.join(
				agentsDir,
				`${artifact.module}-${artifact.name}.md`,
			);
		...
		for (const workflow of workflows) {
			...
			const targetPath = path.join(
				workflowsDir,
				`${workflow.module}-${workflow.name}.md`,
			);
			await this.writeFile(targetPath, commandContent);
		}

		const _totalInstalled =
			agentArtifacts.length + tasks.length + tools.length + workflows.length;
		...
		console.log(
			chalk.dim(`  - Location: ${path.relative(projectDir, location)}`),
		);
		console.log(
			chalk.yellow(
...
`;
	...
	createWorkflowCommand(workflow, content) {
		const description =
			workflow.description || `Execute the ${workflow.name} workflow`;
