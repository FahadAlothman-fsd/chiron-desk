...
class WindsurfSetup extends BaseIdeSetup {
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		const agents = agentArtifacts.map((a) => ({
			module: a.module,
			name: a.name,
		}));
		...
		for (const item of [...agents, ...tasks, ...tools, ...workflows])
		...
		for (const artifact of agentArtifacts) {
			const processedContent = this.createWorkflowContent(
		...
		for (const workflow of workflows) {
			...
			const processedContent = this.createWorkflowWorkflowContent(
				workflow,
				content,
			);
		...
		console.log(
			chalk.dim(`  - Organized in modules: ${[...modules].join(", ")}`),
		);
		console.log(
			chalk.dim(
				`  - Workflows directory: ${path.relative(projectDir, workflowsDir)}`,
			),
		);
		...
		if (options.showHints !== false) {
			...
			console.log(
				chalk.dim("  - auto_execution_mode: 3 (recommended for agents)"),
			);
			console.log(
				chalk.dim("  - auto_execution_mode: 2 (recommended for tasks/tools)"),
			);
			console.log(
				chalk.dim("  - auto_execution_mode: 1 (recommended for workflows)"),
			);
			console.log(
				chalk.dim("  - Workflows can be triggered via the Windsurf menu"),
			);
		}
