...
class CodexSetup extends BaseIdeSetup {
			...
			const { location } = await inquirer.prompt([
				{
					...
					choices: [
						{
							name:
								"Global - Simple for single project " +
								"(~/.codex/prompts, but references THIS project only)",
							value: "global",
						},
						{
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		const installLocation =
			options.preCollectedConfig?.installLocation || "global";

		const { artifacts, counts } = await this.collectClaudeArtifacts(
			projectDir,
			bmadDir,
			options,
		);
		...
		console.log(
			chalk.dim(`  - ${counts.workflows} workflow commands exported`),
		);
		if (counts.workflowLaunchers > 0) {
			console.log(
				chalk.dim(
					`  - ${counts.workflowLaunchers} workflow launchers exported`,
				),
			);
		}
	...
	async detect(projectDir) {
		...
		const projectSpecificDir = this.getCodexPromptDir(
			projectDir_local,
			"project",
		);
	 ...
	 */
	getGlobalInstructions(_destDir) {
		const lines = [
			"",
			...
			chalk.dim(
				"  To use with other projects, you'd need to copy the .bmad dir",
			),
			"",
	 ...
	 * @returns {string} Instructions text
	 */
	getProjectSpecificInstructions(_projectDir = null, destDir = null) {
		...
		const commonLines = [
			...
			chalk.bold.cyan("═".repeat(70)),
			"",
			chalk.white("  Prompts will be installed to: ") +
				chalk.cyan(destDir || "<project>/.codex/prompts"),
			"",
			chalk.bold.yellow(
		...
		const lines = [
			...commonLines,
			...(isWindows ? windowsLines : unixLines),
			...closingLines,
		];
