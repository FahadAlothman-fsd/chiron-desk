...
class GitHubCopilotSetup extends BaseIdeSetup {
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		for (const artifact of agentArtifacts) {
			...
			const chatmodeContent = await this.createChatmodeContent(
			...
			const targetPath = path.join(
				chatmodesDir,
				`bmad-agent-${artifact.module}-${artifact.name}.chatmode.md`,
			);
			...
			console.log(
				chalk.green(
					`  ✓ Created chat mode: bmad-agent-${artifact.module}-${artifact.name}`,
				),
			);
		}
		...
		console.log(
			chalk.dim(
				`  - Chat modes directory: ${path.relative(projectDir, chatmodesDir)}`,
			),
		);
	...
	async configureVsCodeSettings(projectDir, options) {
		...
		if (configChoice === "defaults") {
			...
			bmadSettings = {
				"chat.agent.enabled": true,
				"chat.agent.maxRequests": Number.parseInt(manual.maxRequests || 15),
...
`;
	...
	async cleanup(projectDir) {
		...
		if (await fs.pathExists(chatmodesDir)) {
			...
			if (removed > 0) {
				console.log(
					chalk.dim(`  Cleaned up ${removed} existing BMAD chat modes`),
				);
			}
		}
	}
...
`;
		...
		const chatmodePath = path.join(
			chatmodesDir,
			`bmad-agent-custom-${agentName}.chatmode.md`,
		);
