...
class IdeManager {
			...
			for (const file of files) {
				...
				try {
					...
					const HandlerClass =
						...
						HandlerModule[Object.keys(HandlerModule)[0]];
					...
					if (HandlerClass) {
						...
						if (instance.name && typeof instance.name === "string") {
							...
							console.log(
								chalk.yellow(
									`  Warning: ${moduleName} handler missing valid 'name' property`,
								),
							);
						}
					}
					...
					console.log(
						chalk.yellow(
							`  Warning: Could not load ${moduleName}: ${error.message}`,
						),
					);
				}
			}
	...
	async setup(ideName, projectDir, bmadDir, options = {}) {
		...
		if (!handler) {
			...
			console.log(
				chalk.dim("Supported IDEs:", [...this.handlers.keys()].join(", ")),
			);
	...
	async detectInstalledIdes(projectDir) {
		...
		for (const [name, handler] of this.handlers) {
			if (
				typeof handler.detect === "function" &&
				(await handler.detect(projectDir))
			) {
				detected.push(name);
			}
		}
	 ...
	 */
	async installCustomAgentLaunchers(
		ides,
		projectDir,
		agentName,
		agentPath,
		...
		for (const ideName of ides) {
			...
			if (!handler) {
				console.warn(
					chalk.yellow(
						`⚠️  IDE '${ideName}' is not yet supported for custom agent installation`,
					),
				);
				continue;
			}
			...
			try {
				if (typeof handler.installCustomAgentLauncher === "function") {
					const result = await handler.installCustomAgentLauncher(
						projectDir,
						agentName,
						agentPath,
						metadata,
				...
				console.warn(
					chalk.yellow(
						`⚠️  Failed to install ${ideName} launcher: ${error.message}`,
					),
				);
			}
		}
