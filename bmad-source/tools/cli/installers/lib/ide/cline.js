...
class ClineSetup extends BaseIdeSetup {
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		const { artifacts, counts } = await this.collectClineArtifacts(
			projectDir,
			bmadDir,
			options,
		);
		...
		const written = await this.flattenAndWriteArtifacts(
			artifacts,
			workflowsDir,
		);
		...
		console.log(
			chalk.dim(`  - ${counts.workflows} workflow commands installed`),
		);
		if (counts.workflowLaunchers > 0) {
			console.log(
				chalk.dim(
					`  - ${counts.workflowLaunchers} workflow launchers installed`,
				),
			);
		}
		console.log(
			chalk.dim(
				`  - ${written} files written to ${path.relative(projectDir, workflowsDir)}`,
			),
		);
		...
		console.log(
			chalk.cyan("  BMAD workflows are available as slash commands in Cline"),
		);
