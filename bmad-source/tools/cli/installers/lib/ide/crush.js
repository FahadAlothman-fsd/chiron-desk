...
class CrushSetup extends BaseIdeSetup {
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		// Organize by module
		const agentCount = await this.organizeByModule(
			commandsDir,
			agentArtifacts,
			tasks,
			tools,
		...
		console.log(
			chalk.dim(`  - ${agentCount.workflows} workflow commands created`),
		);
		console.log(
			chalk.dim(
	...
	createTaskCommand(task, content) {
		...
		const commandContent = `# /task-${task.name} Command
...
`;
	...
	createToolCommand(tool, content) {
		...
		const commandContent = `# /tool-${tool.name} Command
...
`;
	...
	createWorkflowCommand(workflow, content) {
		const workflowName = workflow.name
			? this.formatTitle(workflow.name)
			: "Workflow";

		const commandContent = `# /${workflow.name} Command
