...
class QwenSetup extends BaseIdeSetup {
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		const tasks = await getTasksFromBmad(
			bmadDir,
			options.selectedModules || [],
		);
		...
		for (const item of [...agentArtifacts, ...tasks, ...tools, ...workflows])
		...
		for (const artifact of agentArtifacts) {
			...
			console.log(
				chalk.green(
					`  ✓ Added agent: /bmad:${artifact.module}:agents:${artifact.name}`,
				),
			);
		}
		...
		for (const task of tasks) {
			...
			taskCount++;
			console.log(
				chalk.green(`  ✓ Added task: /bmad:${task.module}:tasks:${task.name}`),
			);
		}
		...
		for (const tool of tools) {
			...
			toolCount++;
			console.log(
				chalk.green(`  ✓ Added tool: /bmad:${tool.module}:tools:${tool.name}`),
			);
		}
		...
		for (const workflow of workflows) {
			...
			console.log(
				chalk.green(
					`  ✓ Added workflow: /bmad:${workflow.module}:workflows:${workflow.name}`,
				),
			);
		}
	...
	async updateSettings(qwenDir) {
		...
		if (await fs.pathExists(settingsPath)) {
			try {
				...
				if (
					...
					settings.contextFileName = settings.contextFileName.filter(
						(fileName) =>
							...
							!fileName.startsWith("bmad-method/"),
					);
				...
				console.warn(
					chalk.yellow("  ⚠ Could not update settings.json:"),
					error.message,
				);
			}
		}
	...
	processAgentLauncherContent(launcherContent, metadata = {}) {
		...
		const contentWithoutFrontmatter = launcherContent.replace(
			frontmatterRegex,
			"",
		);
...
`;
	...
	processContent(content, metadata = {}) {
		...
		const prompt = super.processContent(content, metadata);
		...
		const isWorkflow =
			content.includes("workflow:") || content.includes("name:");
...
`;
	...
	async cleanup(projectDir) {
		...
		const oldBmadMethodDir = path.join(
			projectDir,
			this.configDir,
			"bmad-method",
		);
