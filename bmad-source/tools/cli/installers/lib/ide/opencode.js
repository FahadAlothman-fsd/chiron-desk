	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		for (const artifact of agentArtifacts) {
			...
			const targetPath = path.join(
				agentsBaseDir,
				`bmad-agent-${artifact.module}-${artifact.name}.md`,
			);
		...
		for (const artifact of workflowArtifacts) {
			if (artifact.type === "workflow-command") {
				...
				const targetPath = path.join(
					commandsBaseDir,
					`bmad-workflow-${artifact.module}-${workflowName}.md`,
				);
		...
		const { tasks, tools } = await this.generateFlatTaskToolCommands(
			bmadDir,
			commandsBaseDir,
		);
		...
		console.log(
			chalk.dim(`  - ${agentCount} agents installed to .opencode/agent/`),
		);
		if (workflowCommandCount > 0) {
			console.log(
				chalk.dim(
					`  - ${workflowCommandCount} workflows installed to .opencode/command/`,
				),
			);
		}
		if (tasks + tools > 0) {
			console.log(
				chalk.dim(
					`  - ${tasks + tools} tasks/tools installed to .opencode/command/ (${tasks} tasks, ${tools} tools)`,
				),
			);
		}
...
`;
		...
		const launcherPath = path.join(
			agentsDir,
			`bmad-agent-custom-${agentName}.md`,
		);
