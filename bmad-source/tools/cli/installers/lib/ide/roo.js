...
class RooSetup extends BaseIdeSetup {
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		if (skippedCount > 0) {
			console.log(
				chalk.dim(`  - ${skippedCount} modes skipped (already exist)`),
			);
		}
	...
	async createModeEntry(agent, content, permissionChoice, projectDir) {
		...
		const relativePath = path
			.relative(projectDir, agent.path)
			.replaceAll("\\", "/");
	...
	getPermissionsForAgent(agent, permissionChoice) {
		if (permissionChoice === "custom") {
			...
			if (agent.name.includes("doc") || agent.name.includes("write")) {
				return this.defaultPermissions.docs;
			}
			if (agent.name.includes("config") || agent.name.includes("setup")) {
				return this.defaultPermissions.config;
			}
			if (agent.name.includes("style") || agent.name.includes("css")) {
	...
	async cleanup(projectDir) {
		...
		if (await fs.pathExists(roomodesPath)) {
			...
			console.log(
				chalk.dim(`Removed ${removedCount} BMAD modes from .roomodes`),
			);
		}
	}
