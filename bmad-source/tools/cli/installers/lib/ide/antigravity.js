...
class AntigravitySetup extends BaseIdeSetup {
							...
							const locationAnswer = await inquirer.prompt([
								{
									...
									name: "location",
									message:
										"Where would you like to install Antigravity subagents?",
									choices: [
										{
											name: "Project level (.agent/agents/)",
											value: "project",
										},
	...
	async cleanup(projectDir) {
		const bmadWorkflowsDir = path.join(
			projectDir,
			this.configDir,
			this.workflowsDir,
			"bmad",
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		console.log(
			chalk.dim(
				`  - Workflows directory: ${path.relative(projectDir, bmadWorkflowsDir)}`,
			),
		);
		console.log(
			chalk.yellow(
		...
		const { install } = await inquirer.prompt([
			{
				...
				name: "install",
				message:
					"Would you like to install Antigravity subagents for enhanced functionality?",
				choices: [
	...
	async copySelectedSubagents(
		...
		if (location === "user") {
			...
			console.log(
				chalk.dim("  Installing subagents globally to: ~/.agent/agents/"),
			);
		} else {
			targetDir = path.join(projectDir, ".agent", "agents");
			console.log(
				chalk.dim("  Installing subagents to project: .agent/agents/"),
			);
		}
