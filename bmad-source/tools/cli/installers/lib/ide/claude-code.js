...
} = require("./shared/workflow-command-generator");
const {
	TaskToolCommandGenerator,
} = require("./shared/task-tool-command-generator");
...
class ClaudeCodeSetup extends BaseIdeSetup {
		...
		for (const moduleName of modules) {
			...
			if (await this.exists(injectionConfigPath)) {
				...
				try {
							...
							const locationAnswer = await inquirer.prompt([
								{
									...
									name: "location",
									message:
										"Where would you like to install Claude Code subagents?",
									choices: [
										{
											name: "Project level (.claude/agents/)",
											value: "project",
										},
					...
					console.log(
						chalk.yellow(
							`  Warning: Failed to process ${moduleName} features: ${error.message}`,
						),
					);
				}
			}
	...
	async cleanup(projectDir) {
		const bmadCommandsDir = path.join(
			projectDir,
			this.configDir,
			this.commandsDir,
			"bmad",
	...
	async setup(projectDir, bmadDir, options = {}) {
		...
		const { artifacts: agentArtifacts, counts: agentCounts } =
			await agentGen.collectAgentArtifacts(
				bmadDir,
		...
		const agentCount = await agentGen.writeAgentLaunchers(
			bmadCommandsDir,
			agentArtifacts,
		);
		...
		if (options.preCollectedConfig?._alreadyConfigured) {
		...
		for (const artifact of workflowArtifacts) {
			if (artifact.type === "workflow-command") {
				...
				const commandPath = path.join(
					moduleWorkflowsDir,
					path.basename(artifact.relativePath),
				);
		...
		const taskToolResult = await taskToolGen.generateTaskToolCommands(
			projectDir,
			bmadDir,
		);
		...
		if (workflowCommandCount > 0) {
			console.log(
				chalk.dim(`  - ${workflowCommandCount} workflow commands generated`),
			);
		}
		...
		console.log(
			chalk.dim(
				`  - Commands directory: ${path.relative(projectDir, bmadCommandsDir)}`,
			),
		);
	...
	async getAgentsFromSource(sourceDir, selectedModules) {
		...
		if (await fs.pathExists(path.join(corePath, "agents"))) {
			const coreAgents = await getAgentsFromDir(
	 ...
	 */
	async processModuleInjectionsWithConfig(
		projectDir,
		_bmadDir,
		options,
		preCollectedConfig,
	 ...
	 */
	async processModuleInjections(projectDir, _bmadDir, options) {
		// Get list of installed modules
		const modules = options.selectedModules || [];
		...
		const { install } = await inquirer.prompt([
			{
				...
				name: "install",
				message:
					"Would you like to install Claude Code subagents for enhanced functionality?",
				choices: [
		...
		if (await this.exists(targetPath)) {
			...
			if (content.includes(marker)) {
				...
				if (
					...
					injectionContent = filterAgentInstructions(
						injection.content,
						subagentChoices.selected,
					);
				}
				...
				console.log(
					chalk.dim(`    Injected: ${injection.point} → ${injection.file}`),
				);
			}
		}
	}
	...
	async copySelectedSubagents(
		...
		if (location === "user") {
			...
			console.log(
				chalk.dim("  Installing subagents globally to: ~/.claude/agents/"),
			);
		} else {
			targetDir = path.join(projectDir, ".claude", "agents");
			console.log(
				chalk.dim("  Installing subagents to project: .claude/agents/"),
			);
		}
		...
		const resolvedFiles = await resolveSubagentFiles(
			handlerBaseDir,
			subagentConfig,
			choices,
		);
		...
		for (const resolved of resolvedFiles) {
			try {
				...
				console.log(
					chalk.yellow(
						`    ⚠ Error copying ${resolved.file}: ${error.message}`,
					),
				);
			}
		}
