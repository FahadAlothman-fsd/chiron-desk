...
class BaseIdeSetup {
		...
		for (const entry of entries) {
			if (
				...
				if (await fs.pathExists(moduleAgentsPath)) {
					const moduleAgents = await this.scanDirectory(
						moduleAgentsPath,
						".md",
					);
					agents.push(
	...
	async getTasks(bmadDir, standaloneOnly = false) {
		...
		if (await fs.pathExists(coreTasksPath)) {
			const coreTasks = await this.scanDirectoryWithStandalone(coreTasksPath, [
				".md",
				".xml",
			]);
			tasks.push(
		...
		for (const entry of entries) {
			if (
				...
				if (await fs.pathExists(moduleTasksPath)) {
					const moduleTasks = await this.scanDirectoryWithStandalone(
						moduleTasksPath,
						[".md", ".xml"],
					);
					tasks.push(
	...
	async getTools(bmadDir, standaloneOnly = false) {
		...
		if (await fs.pathExists(coreToolsPath)) {
			const coreTools = await this.scanDirectoryWithStandalone(coreToolsPath, [
				".md",
				".xml",
			]);
			tools.push(
		...
		for (const entry of entries) {
			if (
				...
				if (await fs.pathExists(moduleToolsPath)) {
					const moduleTools = await this.scanDirectoryWithStandalone(
						moduleToolsPath,
						[".md", ".xml"],
					);
					tools.push(
			...
			if (
				...
				if (await fs.pathExists(moduleWorkflowsPath)) {
					const moduleWorkflows =
						await this.findWorkflowYamlFiles(moduleWorkflowsPath);
					workflows.push(
