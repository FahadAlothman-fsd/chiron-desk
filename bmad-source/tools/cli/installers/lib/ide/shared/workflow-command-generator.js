...
class WorkflowCommandGenerator {
	constructor(bmadFolderName = "bmad") {
		this.templatePath = path.join(
			__dirname,
			"../templates/workflow-command-template.md",
		);
	...
	async generateWorkflowCommands(projectDir, bmadDir) {
		...
		if (!workflows) {
			console.log(
				chalk.yellow(
					"Workflow manifest not found. Skipping command generation.",
				),
			);
			return { generated: 0 };
		}
		...
		const standaloneWorkflows = workflows.filter(
			(w) => w.standalone === "true" || w.standalone === true,
		);

		// Base commands directory
		const baseCommandsDir = path.join(
			projectDir,
			".claude",
			"commands",
			"bmad",
		...
		for (const workflow of standaloneWorkflows) {
			...
			const commandContent = await this.generateCommandContent(
				workflow,
				bmadDir,
			);
