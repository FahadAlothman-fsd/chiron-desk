...
class Installer {
	 ...
	 */
	async copyFileWithPlaceholderReplacement(
		sourcePath,
		targetPath,
		bmadFolderName,
	) {
...
If AgentVibes party mode is enabled, immediately trigger TTS with agent's voice:
   ...
   Run in background (&) to avoid blocking`,
	...
	async collectToolConfigurations(
		...
		const savedIdeConfigs =
			await this.ideConfigManager.loadAllIdeConfigs(bmadDir);
		...
		if (!toolConfig.skipIde && toolConfig.ides && toolConfig.ides.length > 0) {
				...
				for (const ide of newlySelectedIdes) {
					...
					if (needsPrompts) {
						...
						try {
							...
							console.warn(
								chalk.yellow(
									`Warning: Could not load configuration for ${ide}`,
								),
							);
						}
					}
			...
			const keptIdes = toolConfig.ides.filter((ide) =>
				previouslyConfiguredIdes.includes(ide),
			);
	...
	async install(config) {
		...
		const _projectDir = path.resolve(config.directory);
		...
		const bmadFolderName = moduleConfigs.core?.bmad_folder
			? moduleConfigs.core.bmad_folder
			: "bmad";
		...
		try {
			...
			const _targetBmadDir = path.join(projectDir, bmadFolderName);

			// If bmad_folder changed during update/upgrade, back up old folder and do fresh install
			if (
				existingBmadDir &&
				(await fs.pathExists(existingBmadDir)) &&
				existingBmadFolderName !== bmadFolderName
			) {
				spinner.stop();
				console.log(
					chalk.yellow(
				...
				console.log(
					chalk.yellow(
						"This will result in a fresh installation to the new folder.",
					),
				);
				...
				const { confirmFreshInstall } = await inquirer.prompt([
					{
						...
						message: chalk.cyan(
							"Proceed with fresh install? (Your old folder will be backed up)",
						),
						default: true,
					},
				]);
				...
				spinner.succeed(
					`Backed up ${existingBmadFolderName} → ${path.basename(backupDir)}`,
				);
				console.log(chalk.cyan("\n📋 Important:"));
				console.log(
					chalk.dim(
						`  - Your old installation has been backed up to: ${path.basename(backupDir)}`,
					),
				);
				console.log(
					chalk.dim(
						"  - If you had custom agents or configurations, copy them from:",
					),
				);
			...
			if (existingInstall.installed && !config.force && !config._quickUpdate) {
				...
				if (action === "reinstall") {
					...
					console.log(
						chalk.red(
							"All custom files and modifications in the bmad directory will be lost.",
						),
					);
					...
					const { confirmReinstall } = await inquirer.prompt([
						{
							...
							message: chalk.yellow(
								"Are you sure you want to delete and reinstall?",
							),
							default: false,
						},
					]);
					...
					console.log(
						chalk.dim(
							`DEBUG: Read ${existingFilesManifest.length} files from manifest`,
						),
					);
					console.log(
						chalk.dim(
							`DEBUG: Manifest has hashes: ${existingFilesManifest.some((f) => f.hash)}`,
						),
					);
					...
					if (modifiedFiles.length > 0) {
						const tempModifiedBackupDir = path.join(
							projectDir,
							".bmad-modified-backup-temp",
						);
						...
						spinner.start(
							`Backing up ${modifiedFiles.length} modified files...`,
						);
						for (const modifiedFile of modifiedFiles) {
							const relativePath = path.relative(bmadDir, modifiedFile.path);
							const tempBackupPath = path.join(
								tempModifiedBackupDir,
								relativePath,
							);
							console.log(
								chalk.dim(`DEBUG: Backing up ${relativePath} to temp`),
							);
			...
			if (config._quickUpdate) {
				...
				const preSelectedIdes =
					config.ides && config.ides.length > 0 ? config.ides : null;
			...
			const _modulesToInstall = config.installCore
				? ["core", ...config.modules]
				: config.modules;
			...
			if (config.installCore || resolution.byModule.core) {
				...
				await this.installCoreWithDependencies(
					bmadDir,
					resolution.byModule.core,
				);
			...
			const _allModulesToList = config._preserveModules
				? [...(config.modules || []), ...config._preserveModules]
				: config.modules || [];
			...
			if (!config.skipIde && config.ides && config.ides.length > 0) {
				...
				const validIdes = config.ides.filter(
				...
				if (validIdes.length === 0) {
					console.log(
						chalk.yellow(
							"⚠️  No valid IDEs selected. Skipping IDE configuration.",
						),
					);
				} else {
					// Check if any IDE might need prompting (no pre-collected config)
					const needsPrompting = validIdes.some(
						(ide) => !ideConfigurations[ide],
					);
					...
					for (const ide of validIdes) {
						...
						if (
							ideConfigurations[ide] &&
							!ideConfigurations[ide]._alreadyConfigured
						) {
							await this.ideConfigManager.saveIdeConfig(
								bmadDir,
								ide,
								ideConfigurations[ide],
							);
					...
					if (spinner.isSpinning) {
						spinner.succeed(
							`Configured ${validIdes.length} IDE${validIdes.length > 1 ? "s" : ""}`,
						);
					} else {
						console.log(
							chalk.green(
				...
				const validIdesForDocs = (config.ides || []).filter(
					(ide) => ide && typeof ide === "string",
				);
			...
			if (config._isUpdate) {
				if (config._customFiles && config._customFiles.length > 0) {
					spinner.start(
						`Restoring ${config._customFiles.length} custom files...`,
					);
					...
					if (
						config._tempBackupDir &&
						(await fs.pathExists(config._tempBackupDir))
					) {
						await fs.remove(config._tempBackupDir);
					}

					spinner.succeed(
						`Restored ${config._customFiles.length} custom files`,
					);
				...
				if (config._modifiedFiles && config._modifiedFiles.length > 0) {
					...
					if (
						...
						spinner.start(
							`Restoring ${modifiedFiles.length} modified files as .bak...`,
						);
						...
						spinner.succeed(
							`Restored ${modifiedFiles.length} modified files as .bak`,
						);
					}
				}
			}
			...
			if (customFiles.length > 0) {
				...
				console.log(
					chalk.dim("The following custom files were found and restored:\n"),
				);
			...
			if (modifiedFiles.length > 0) {
				...
				console.log(
					chalk.dim(
						"The following files were modified and backed up with .bak extension:\n",
					),
				);
				for (const file of modifiedFiles) {
					console.log(
						chalk.dim(`  - ${file.relativePath} → ${file.relativePath}.bak`),
					);
				}
				console.log(
				...
				console.log(
					chalk.dim(
						"Review the .bak files to see your changes and merge if needed.\n",
					),
				);
			}

			// Reinstall custom agents from _cfg/custom/agents/ sources
			const customAgentResults = await this.reinstallCustomAgents(
				projectDir,
				bmadDir,
			);
			...
			if (!config.skipCleanup && config._isUpdate) {
				try {
					...
					if (cleanupResult.retained > 0) {
						console.log(
							chalk.dim(`Run 'bmad cleanup' anytime to manage retained files`),
						);
					}
				} catch (cleanupError) {
					...
					console.log(
						chalk.dim('Run "bmad cleanup" to manually clean up legacy files'),
					);
				}
			}
	...
	async generateModuleConfigs(bmadDir, moduleConfigs) {
		...
		const installedModules = entries
			.filter(
				(entry) =>
					entry.isDirectory() && entry.name !== "_cfg" && entry.name !== "docs",
			)
...
`;
				...
				if (
					moduleName !== "core" &&
					coreConfig &&
					Object.keys(coreConfig).length > 0
				) {
				...
				if (coreSection && moduleName !== "core") {
					...
					for (const line of lines) {
						...
						if (Object.hasOwn(coreConfig, key)) {
	...
	async installCoreWithDependencies(bmadDir, coreFiles) {
		const _sourcePath = getModulePath("core");
		const _targetPath = path.join(bmadDir, "core");
	 ...
	 */
	async installModuleWithDependencies(moduleName, bmadDir, _moduleFiles) {
		...
		if (files.agents && files.agents.length > 0) {
			...
			for (const agentPath of files.agents) {
				...
				if (await fs.pathExists(sourcePath)) {
					await this.copyFileWithPlaceholderReplacement(
						sourcePath,
						targetPath,
						this.bmadFolderName || "bmad",
					);
	...
	async copyDirectoryWithFiltering(sourcePath, targetPath) {
		...
		for (const file of files) {
			...
			if (file.includes("agents/") && file.endsWith(".md")) {
				...
				if (agentMatch) {
					console.log(
						chalk.dim(`  Skipping web-only agent: ${path.basename(file)}`),
					);
	...
	async processAgentFiles(modulePath, moduleName) {
		...
		const _projectDir = path.dirname(bmadDir);
		...
		for (const agentFile of agentFiles) {
			...
			if (agentFile.endsWith(".agent.yaml")) {
				...
				const customizePath = path.join(
					cfgAgentsDir,
					`${moduleName}-${agentName}.customize.yaml`,
				);
				...
				if (!(await fs.pathExists(customizePath))) {
					...
					if (await fs.pathExists(genericTemplatePath)) {
						...
						console.log(
							chalk.dim(
								`  Created customize: ${moduleName}-${agentName}.customize.yaml`,
							),
						);
					}
				}
	 ...
	 */
	async buildStandaloneAgents(bmadDir, _projectDir) {
		...
		const agentDirs = await fs.readdir(standaloneAgentsPath, {
			withFileTypes: true,
		});
			...
			if (customizeExists) {
				...
				if (customizeYaml) {
					...
					if (
						...
						customizeYaml.critical_actions.length > 0
					) {
	...
	async compileAgents(config) {
		...
		try {
			...
			if (customAgentResults.count > 0) {
				spinner.succeed(
					`Rebuilt ${customAgentResults.count} custom agent${customAgentResults.count > 1 ? "s" : ""}`,
				);
			...
			if (existingIdes && existingIdes.length > 0) {
				...
				console.log(
					chalk.yellow("⚠️  No IDEs configured. Skipping IDE update."),
				);
			}
	...
	async quickUpdate(config) {
		...
		try {
			...
			const modulesToUpdate = installedModules.filter((id) =>
				availableModuleIds.has(id),
			);
			const skippedModules = installedModules.filter(
				(id) => !availableModuleIds.has(id),
			);

			spinner.succeed(
				`Found ${modulesToUpdate.length} module(s) to update and ${configuredIdes.length} configured tool(s)`,
			);

			if (skippedModules.length > 0) {
				console.log(
					chalk.yellow(
						`⚠️  Skipping ${skippedModules.length} module(s) - no source available: ${skippedModules.join(", ")}`,
					),
				);
			}
			...
			const corePrompted = await this.configCollector.collectModuleConfigQuick(
				"core",
				projectDir,
				true,
			);
			...
			for (const moduleName of modulesToUpdate) {
				const modulePrompted =
					await this.configCollector.collectModuleConfigQuick(
						moduleName,
						projectDir,
						true,
					);
			...
			if (!promptedForNewFields) {
				console.log(
					chalk.green(
						"✓ All configuration is up to date, no new options to configure",
					),
				);
			}
			...
			const newBmadFolderName =
				this.configCollector.collectedConfig.core?.bmad_folder ||
				existingBmadFolderName;
			...
			if (existingBmadFolderName === newBmadFolderName) {
				...
				console.log(
					chalk.yellow("The installer will handle the folder migration.\n"),
				);
			}
			...
			const _result = await this.install(installConfig);
	...
	async handleLegacyV4Migration(projectDir, legacyV4) {
		...
		console.log(
			chalk.dim("The installer found legacy artefacts in your project.\n"),
		);
		...
		if (otherOffenders.length > 0) {
			...
			console.log(
				chalk.dim(
					"It is recommended to remove the following items before proceeding:\n",
				),
			);
			...
			for (const p of otherOffenders)
				console.log(chalk.dim(`  rmdir /S /Q "${p}"`));
			...
			if (cleanedUp) {
				...
				console.log(
					chalk.yellow("⚠️  Proceeding without recommended cleanup\n"),
				);
			}
		}
		...
		if (bmadFolders.length > 0) {
			console.log(
				chalk.cyan("The following legacy folders will be moved to v4-backup:"),
			);
			...
			if (proceed) {
				...
				for (const folder of bmadFolders) {
					...
					console.log(
						chalk.green(
							`✓ Moved ${folderName} to ${path.relative(projectDir, finalBackupPath)}`,
						),
					);
				}
			} else {
	...
	async readFilesManifest(bmadDir) {
		...
		try {
			...
			console.warn(
				"Warning: Could not read files-manifest.csv:",
				error.message,
			);
			return [];
		}
				...
				for (const entry of entries) {
					...
					if (entry.isDirectory()) {
						...
						if (!fileInfo) {
							...
							const currentHash =
								await this.manifest.calculateFileHash(fullPath);
	...
	async createAgentConfigs(bmadDir, userInfo = null) {
				...
				if (await fs.pathExists(moduleAgentsPath)) {
					...
					for (const agentFile of agentFiles) {
						if (agentFile.endsWith(".md")) {
							...
							const hasLocalSkip = agentContent.match(
								/<agent[^>]*\slocalskip="true"[^>]*>/,
							);
							if (hasLocalSkip) {
							...
							const agentConfigNodes =
								this.extractAgentConfigNodes(agentContent);
							...
							if (details) {
								const configPath = path.join(
									agentConfigDir,
									`${entry.name}-${agentName}.md`,
								);
								if (await fs.pathExists(configPath)) {
									const configContent = await fs.readFile(configPath, "utf8");
									details = AgentPartyGenerator.applyConfigOverrides(
										details,
										configContent,
									);
								}
		...
		const templatePath = getSourcePath(
			"utility",
			"models",
		...
		for (const agent of agents) {
			...
			if (agent.agentConfigNodes && agent.agentConfigNodes.length > 0) {
				...
				const commentPattern =
					/(\s*<!-- Agent-specific configuration nodes -->)/;
	...
	async generateAgentManifest(bmadDir, agentDetails) {
		...
		await AgentPartyGenerator.writeAgentParty(manifestPath, agentDetails, {
			forWeb: false,
		});
	}
	...
	extractAgentConfigNodes(content) {
		...
		try {
			...
			const selfClosingPattern =
				/<([a-zA-Z][a-zA-Z0-9_-]*)\s+[^>]*agentConfig="true"[^>]*\/>/g;
			...
			while ((match = withContentPattern.exec(content)) !== null) {
				...
				const openingTagMatch = fullMatch.match(
					new RegExp(`<${tagName}([^>]*)>`),
				);
				if (openingTagMatch) {
					const attributes = openingTagMatch[1].replace(
						/\s*agentConfig="true"/,
						"",
					);
	...
	async reinstallCustomAgents(projectDir, bmadDir) {
		...
		const foundAgents = [];
		const processedAgents = new Set(); // Track to avoid duplicates
		...
		for (const location of sourceLocations) {
			if (await fs.pathExists(location)) {
				...
				const newAgents = agents.filter(
					(agent) => !processedAgents.has(agent.name),
				);
		...
		try {
			...
			for (const agent of foundAgents) {
				try {
					...
					const cfgAgentsBackupDir = path.join(
						bmadDir,
						"_cfg",
						"custom",
						"agents",
					...
					const backupYamlPath = path.join(
						cfgAgentsBackupDir,
						`${finalAgentName}.agent.yaml`,
					);
					...
					if (await fs.pathExists(manifestFile)) {
						...
						const manifestData = extractManifestData(
							xml,
							manifestMetadata,
							relativePath,
							"custom",
					...
					console.log(
						chalk.yellow(
							`  ⚠️  Failed to reinstall ${agent.name}: ${agentError.message}`,
						),
					);
				}
			}
			...
			console.log(
				chalk.yellow(`  ⚠️  Error reinstalling custom agents: ${error.message}`),
			);
		}
	...
	async scanForLegacyFiles(bmadDir) {
		...
		try {
			...
			console.warn(
				`Warning: Could not scan for legacy files: ${error.message}`,
			);
		}
	...
	isExpectedFile(relativePath, manifestFiles) {
		...
		// Custom files
		if (
			relativePath.startsWith("custom/") ||
		...
		if (
			...
			relativePath === "files-manifest.csv"
		) {
			return true;
		}
	...
	isDocumentationFile(relativePath) {
		...
		return (
			docExtensions.some((ext) => relativePath.endsWith(ext)) ||
			docPatterns.some((pattern) => relativePath.includes(pattern))
		);
	}
	...
	suggestAlternative(relativePath) {
		const alternatives = {
			"adv-elicit-methods.csv":
				"Use the new structured workflows in src/modules/",
	...
	async performCleanup(bmadDir, skipInteractive = false) {
		...
		if (skipInteractive) {
			...
			if (legacyFiles.documentation.length > 0) {
				categories.push({
					name: "Documentation",
					files: legacyFiles.documentation,
				});
			}
			if (legacyFiles.deprecated_task.length > 0) {
				categories.push({
					name: "Deprecated Task Files",
					files: legacyFiles.deprecated_task,
				});
			}
			...
			if (!prompt.proceed) {
				return {
					deleted: 0,
					retained: allLegacyFiles.length,
			...
			const selectionPrompt = await inquirer.prompt([
				{
					...
					name: "filesToDelete",
					message:
						"Select files to delete (use SPACEBAR to select, ENTER to continue):",
					choices: allLegacyFiles.map((file) => {
						const isRetained = retentionData.retainedFiles.includes(
							file.relativePath,
						);
		...
		for (const file of filesToDelete) {
			try {
				...
				console.warn(
					`Warning: Could not delete ${file.relativePath}: ${error.message}`,
				);
			}
		}
		...
		const newlyRetained = allLegacyFiles
			.filter((f) => !filesToDelete.includes(f))
			.map((f) => f.relativePath);

		retentionData.retainedFiles = [
			...new Set([...retentionData.retainedFiles, ...newlyRetained]),
		];
