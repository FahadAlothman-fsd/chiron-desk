...
class ManifestGenerator {
	...
	async generateManifests(
		...
		this.modules = [
			...new Set(["core", ...selectedModules, ...preservedModules]),
		];
		...
		if (!Array.isArray(resolvedIdes)) {
			throw new TypeError(
				"ManifestGenerator expected `options.ides` to be an array.",
			);
		}

		// Filter out any undefined/null values from IDE list
		this.selectedIdes = resolvedIdes.filter(
	...
	async collectWorkflows(selectedModules) {
		...
		for (const moduleName of this.updatedModules) {
			...
			if (await fs.pathExists(modulePath)) {
				const moduleWorkflows = await this.getWorkflowsFromPath(
					modulePath,
					moduleName,
				);
			...
			for (const entry of entries) {
				...
				if (entry.isDirectory()) {
					...
					try {
						...
						console.warn(
							`Warning: Failed to parse workflow at ${fullPath}: ${error.message}`,
						);
					}
				}
			}
	...
	async collectAgents(selectedModules) {
		...
		for (const moduleName of this.updatedModules) {
			...
			if (await fs.pathExists(agentsPath)) {
				const moduleAgents = await this.getAgentsFromDir(
					agentsPath,
					moduleName,
				);
		...
		if (await fs.pathExists(standaloneAgentsDir)) {
			...
			for (const agentDir of agentDirs) {
				...
				const standaloneAgents = await this.getAgentsFromDir(
					agentDirPath,
					"standalone",
				);
	...
	async getAgentsFromDir(dirPath, moduleName) {
		...
		for (const file of files) {
			...
			if (
				...
				const installPath =
					moduleName === "core"
						? `${this.bmadFolderName}/core/agents/${file}`
						: `${this.bmadFolderName}/${moduleName}/agents/${file}`;
	...
	async getTasksFromDir(dirPath, moduleName) {
		...
		for (const file of files) {
			...
			if (file.endsWith(".xml") || file.endsWith(".md")) {
				...
				const installPath =
					moduleName === "core"
						? `${this.bmadFolderName}/core/tasks/${file}`
						: `${this.bmadFolderName}/${moduleName}/tasks/${file}`;
	...
	async getToolsFromDir(dirPath, moduleName) {
		...
		for (const file of files) {
			...
			if (file.endsWith(".xml") || file.endsWith(".md")) {
				...
				const installPath =
					moduleName === "core"
						? `${this.bmadFolderName}/core/tools/${file}`
						: `${this.bmadFolderName}/${moduleName}/tools/${file}`;
		...
		try {
			...
			for (const row of dataRows) {
				...
				if (this.preservedModules.includes(moduleValue)) {
					...
					const upgradedRow = this.upgradeRowToSchema(
						cleanColumns,
						oldColumns,
						expectedColumns,
						defaultValues,
			...
			console.warn(
				`Warning: Failed to read existing CSV ${csvPath}:`,
				error.message,
			);
			return [];
		}
