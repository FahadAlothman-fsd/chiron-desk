...
class ConfigCollector {
	...
	async collectAllConfigurations(modules, projectDir) {
		...
		const coreAlreadyCollected =
			this.collectedConfig.core &&
			Object.keys(this.collectedConfig.core).length > 0;
	...
	async collectModuleConfigQuick(moduleName, projectDir, silentMode = true) {
		...
		if (await fs.pathExists(installerConfigPath)) {
			...
			if (this.existingConfig?.[moduleName]) {
				if (!this.collectedConfig[moduleName]) {
					this.collectedConfig[moduleName] = {};
				}
				this.collectedConfig[moduleName] = {
					...this.existingConfig[moduleName],
				};
			}
			return false;
		}
		...
		const configKeys = Object.keys(moduleConfig).filter(
			(key) => key !== "prompt",
		);
		const existingKeys = this.existingConfig?.[moduleName]
			? Object.keys(this.existingConfig[moduleName])
			: [];
		...
		if (silentMode && newKeys.length === 0) {
			...
			CLIUtils.displayModuleNoConfig(
				moduleName,
				moduleConfig.header,
		...
		if (newKeys.length > 0) {
			...
			for (const key of newKeys) {
				const item = moduleConfig[key];
				const question = await this.buildQuestion(
					moduleName,
					key,
					item,
					moduleConfig,
			...
			if (questions.length > 0) {
				...
				CLIUtils.displayModuleConfigHeader(
					moduleName,
					moduleConfig.header,
					moduleConfig.subheader,
				);
		...
		if (typeof result === "string" && value !== undefined) {
			...
			if (typeof result === "string") {
				result = result.replaceAll(/{([^}]+)}/g, (match, configKey) => {
					...
					let configValue =
						this.allAnswers[configKey] || this.allAnswers[`${configKey}`];
					if (!configValue) {
	...
	replacePlaceholders(str, currentModule = null, moduleConfig = null) {
		...
		return str.replaceAll(/{([^}]+)}/g, (match, configKey) => {
			...
			if (
				configKey === "project-root" ||
				configKey === "value" ||
				configKey === "directory_name"
			) {
				return match;
			}
			...
			let configValue =
				this.allAnswers[configKey] || this.allAnswers[`core_${configKey}`];
	...
	async buildQuestion(moduleName, key, item, moduleConfig = null) {
		...
		if (this.existingConfig?.[moduleName]) {
			...
			if (
				...
				existingValue.startsWith("{project-root}/")
			) {
		...
		if (
			...
			const detectedFolder = await this.detectExistingBmadFolder(
				this.currentProjectDir,
			);
			if (detectedFolder) {
		...
		const hasSameModuleReference =
			typeof defaultValue === "string" && defaultValue.match(/{([^}]+)}/);
		...
		if (typeof defaultValue === "string") {
			...
			if (!dynamicDefault) {
				defaultValue = this.replacePlaceholders(
					defaultValue,
					moduleName,
					moduleConfig,
				);
		...
		if (existingValue !== null && existingValue !== undefined) {
			...
			if (typeof exampleText === "string") {
				exampleText = this.replacePlaceholders(
					exampleText,
					moduleName,
					moduleConfig,
				);
