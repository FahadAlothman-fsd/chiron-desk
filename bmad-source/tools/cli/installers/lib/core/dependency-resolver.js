...
class DependencyResolver {
	...
	async resolve(bmadDir, selectedModules = [], options = {}) {
		...
		const primaryFiles = await this.collectPrimaryFiles(
			bmadDir,
			modulesToProcess,
		);
		...
		const resolvedDeps = await this.resolveDependencyPaths(
			bmadDir,
			allDependencies,
		);
		...
		for (const module of modules) {
			...
			if (await fs.pathExists(agentsDir)) {
				...
				for (const file of agentFiles) {
					...
					const hasLocalSkip = content.match(
		...
		for (const file of files) {
			...
			if (frontmatterMatch) {
				try {
					...
					console.warn(
						chalk.yellow(
							`Failed to parse frontmatter in ${file.name}: ${error.message}`,
						),
					);
				}
			}
	...
	parseFileReferences(content) {
		...
		const relativePattern =
			/['"](\.\.?\/[^'"]+\.(md|yaml|yml|xml|json|txt|csv))['"]/g;
		let match;
	...
	async resolveSingleDependency(bmadDir, dep) {
		...
		switch (dep.type) {
					...
					if (depPath.includes("*")) {
						...
						if (await fs.pathExists(fullPath)) {
							...
							this.missingDependencies.add(
								`${depPath} (referenced by ${path.basename(dep.from)})`,
							);
						}
					}
				}
			...
			case "command": {
				...
				const commandPath = await this.resolveCommandToPath(
					bmadDir,
					dep.dependency,
				);
				if (commandPath) {
	...
	async resolveTransitiveDependencies(bmadDir, directDeps) {
		...
		for (const depPath of directDeps) {
			...
			if (
				...
				const resolvedSubDeps = await this.resolveDependencyPaths(
					bmadDir,
					subDeps,
				);
	...
	reportResults(organized, selectedModules) {
		...
		for (const [module, files] of Object.entries(organized)) {
			...
			if (totalFiles > 0) {
				...
				console.log(
					chalk.dim(
						`    Status: ${isSelected ? "Selected" : "Dependencies only"}`,
					),
				);
