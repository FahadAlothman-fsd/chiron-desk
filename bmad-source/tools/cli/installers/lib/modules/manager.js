...
class ModuleManager {
	...
	async copyDirectoryWithPlaceholderReplacement(sourceDir, targetDir) {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				await this.copyDirectoryWithPlaceholderReplacement(
					sourcePath,
					targetPath,
				);
			} else {
	...
	async listAvailable() {
		...
		const entries = await fs.readdir(this.modulesSourcePath, {
			withFileTypes: true,
		});
				...
				if (await fs.pathExists(configToRead)) {
					try {
						...
						moduleInfo.description =
							config.description || moduleInfo.description;
						...
						moduleInfo.defaultSelected =
							config.default_selected === undefined
								? false
								: config.default_selected;
					} catch (error) {
						console.warn(
							`Failed to read config for ${entry.name}:`,
							error.message,
						);
					}
				}
	...
	async install(
		...
		if (!(await fs.pathExists(sourcePath))) {
			throw new Error(
				`Module '${moduleName}' not found in ${this.modulesSourcePath}`,
			);
		}
		...
		if (await fs.pathExists(targetPath)) {
			console.log(
				chalk.yellow(`Module '${moduleName}' already installed, updating...`),
			);
		...
		await this.copyModuleWithFiltering(
			sourcePath,
			targetPath,
	...
	async copyModuleWithFiltering(
		...
		for (const file of sourceFiles) {
			...
			if (
				moduleConfig.install_user_docs === false &&
	...
	async copyWorkflowYamlStripped(sourceFile, targetFile) {
		...
		try {
			...
			console.warn(
				chalk.yellow(
					`  Warning: Could not process ${path.basename(sourceFile)}, copying as-is`,
				),
			);
	...
	async vendorCrossModuleWorkflows(sourcePath, targetPath, moduleName) {
		...
		const yamlFiles = agentFiles.filter(
			(f) => f.endsWith(".agent.yaml") || f.endsWith(".yaml"),
		);
		...
		for (const agentFile of yamlFiles) {
			...
			for (const item of workflowInstallItems) {
				...
				if (!sourceMatch) {
					console.warn(
						chalk.yellow(
							`      Could not parse workflow path: ${sourceWorkflowPath}`,
						),
					);
					continue;
				}
				...
				if (!installMatch) {
					console.warn(
						chalk.yellow(
							`      Could not parse workflow-install path: ${installWorkflowPath}`,
						),
					);
					continue;
				}
				...
				const sourceModulePath = path.join(
					this.modulesSourcePath,
					sourceModule,
				);
				const actualSourceWorkflowPath = path.join(
					sourceModulePath,
					"workflows",
					sourceWorkflowSubPath.replace(/\/workflow\.yaml$/, ""),
				);

				const actualDestWorkflowPath = path.join(
					targetPath,
					"workflows",
					installWorkflowSubPath.replace(/\/workflow\.yaml$/, ""),
				);
				...
				if (!(await fs.pathExists(actualSourceWorkflowPath))) {
					console.warn(
						chalk.yellow(
							`      Source workflow not found: ${actualSourceWorkflowPath}`,
						),
					);
					continue;
				}
				...
				await this.copyDirectoryWithPlaceholderReplacement(
					actualSourceWorkflowPath,
					actualDestWorkflowPath,
				);
	...
	async updateWorkflowConfigSource(workflowYamlPath, newModuleName) {
		...
		const configSourcePattern =
			/config_source:\s*["']?\{project-root\}\/[^/]+\/[^/]+\/config\.yaml["']?/g;
		...
		if (updatedYaml !== yamlContent) {
			...
			console.log(
				chalk.dim(
					`      Updated config_source to: ${this.bmadFolderName}/${newModuleName}/config.yaml`,
				),
			);
		}
	}
	...
	async runModuleInstaller(moduleName, bmadDir, options = {}) {
		...
		try {
			...
			if (typeof moduleInstaller.install === "function") {
				...
				if (!result) {
					console.warn(
						chalk.yellow(`Module installer for ${moduleName} returned false`),
					);
				}
			}
		} catch (error) {
			console.error(
				chalk.red(
					`Error running module installer for ${moduleName}: ${error.message}`,
				),
			);
		}
	}
