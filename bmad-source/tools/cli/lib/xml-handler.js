...
class XmlHandler {
	...
	async injectActivation(agentContent, _metadata = {}) {
		try {
			...
			const xmlBlockMatch = agentContent.match(
				/([\s\S]*?)```xml\n([\s\S]*?)\n```([\s\S]*)/,
			);
			if (xmlBlockMatch) {
	...
	injectActivationSimple(agentContent, metadata = {}) {
		...
		try {
			...
			if (metadata.module && metadata.name) {
				...
				activationBlock = activationBlock.replace(
					"{agent-filename}",
					agentFilename,
				);
			}
	...
	async buildFromYaml(yamlPath, customizePath = null, metadata = {}) {
		try {
			...
			const mergedAgent = await this.yamlBuilder.loadAndMergeAgent(
				yamlPath,
				customizePath,
			);
			...
			// Convert to XML
			const xml = await this.yamlBuilder.convertToXml(
				mergedAgent,
				buildMetadata,
			);
