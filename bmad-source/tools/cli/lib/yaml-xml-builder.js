...
class YamlXmlBuilder {
			...
			if (customizeYaml) {
				...
				if (customizeYaml.prompts) {
					merged.agent.prompts = [
						...(merged.agent.prompts || []),
						...customizeYaml.prompts,
					];
				}

				// Append memories
				if (customizeYaml.memories) {
					merged.agent.memories = [
						...(merged.agent.memories || []),
						...customizeYaml.memories,
					];
				}
			}
		}
	...
	async buildFromYaml(agentYamlPath, customizeYamlPath = null, options = {}) {
		...
		const mergedAgent = await this.loadAndMergeAgent(
			agentYamlPath,
			customizeYamlPath,
		);
		...
		const buildMetadata = {
			...
			sourceHash,
			customizeFile: customizeYamlPath
				? path.basename(customizeYamlPath)
				: null,
			customizeHash,
	...
	async buildAgent(agentYamlPath, customizeYamlPath, outputPath, options = {}) {
		...
		const xml = await this.buildFromYaml(
			agentYamlPath,
			customizeYamlPath,
			options,
		);
