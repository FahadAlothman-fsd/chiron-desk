 ...
 */
function replaceProjectRootInFile(
	filePath,
	projectRoot,
	removeCompletely = false,
) {
...
function replacePlaceholdersInDirectory(
	...
	function processDirectory(currentPath) {
		try {
			...
			console.error(
				`Error processing directory ${currentPath}:`,
				error.message,
			);
			stats.errors++;
		}
 ...
 * Legacy function for backward compatibility
 */
function replaceProjectRootInFiles(
	filePaths,
	projectRoot,
	removeCompletely = false,
) {
	return replacePlaceholdersInFiles(
		filePaths,
		projectRoot,
		"/docs",
		removeCompletely,
...
function processBundleRemoval(bundlePath) {
	...
	const stats = replaceProjectRootInDirectory(
		bundlePath,
		null,
		[".md", ".xml"],
		true,
