...
class UI {
	...
	async promptInstall() {
		...
		CLIUtils.displaySection(
			"BMAD™ Setup",
			`Build More, Architect Dreams v${version}`,
		);
			...
			const promptResult = await inquirer.prompt([
				{
					...
					choices: [
						{
							name: "Quick Update (Settings Preserved)",
							value: "quick-update",
						},
						{
							name: "Modify BMAD Installation (Confirm or change each setting)",
							value: "update",
						},
						{
							name: "Remove BMad Folder and Reinstall (Full clean install - BMad Customization Will Be Lost)",
							value: "reinstall",
						},
						{
							name: "Compile Agents (Quick rebuild of all agent .md files)",
							value: "compile",
						},
		...
		const { installedModuleIds } =
			await this.getExistingInstallation(confirmedDirectory);
		...
		const toolSelection = await this.promptToolSelection(
			confirmedDirectory,
			selectedModules,
		);
	 ...
	 */
	async promptToolSelection(projectDir, _selectedModules) {
		...
		if (configuredIdes.length > 0) {
			...
			for (const ideValue of configuredIdes) {
				...
				const preferredIde = preferredIdes.find(
					(ide) => ide.value === ideValue,
				);
				...
				if (ide) {
					...
					console.log(
						chalk.yellow(
							`⚠️  Previously configured IDE '${ideValue}' is no longer available`,
						),
					);
				}
			}
		...
		const remainingPreferred = preferredIdes.filter(
			(ide) => !processedIdes.has(ide.value),
		);
		...
		const remainingOther = otherIdes.filter(
			(ide) => !processedIdes.has(ide.value),
		);
		...
		while (!userConfirmedNoTools) {
			...
			console.log(
				chalk.red(
					"   You must press SPACEBAR to select items, then ENTER to confirm.",
				),
			);
			console.log(
				chalk.red("   Simply highlighting an item does NOT select it."),
			);
			console.log();
			...
			const { goBack } = await inquirer.prompt([
				{
					...
					message: chalk.yellow(
						"Would you like to go back and select at least one tool?",
					),
					default: true,
				},
			]);
	...
	async getExistingInstallation(directory) {
		...
		const installedModuleIds = new Set(
			existingInstall.modules.map((mod) => mod.id),
		);
	...
	async collectCoreConfig(directory) {
		const {
			ConfigCollector,
		} = require("../installers/lib/core/config-collector");
	...
	async getModuleChoices(installedModuleIds) {
		...
		return availableModules.map((mod) => ({
			...
			checked: isNewInstallation
				? mod.defaultSelected || false
				: installedModuleIds.has(mod.id),
		}));
	}
	...
	async selectModules(moduleChoices) {
		CLIUtils.displaySection(
			"Module Selection",
			"Choose the BMAD modules to install",
		);
			...
			if (stats.isDirectory()) {
				...
				if (files.length > 0) {
					...
					const hasBmadInstall =
						(await fs.pathExists(bmadDir)) &&
						(await fs.pathExists(path.join(bmadDir, "_cfg", "manifest.yaml")));

					console.log(
						chalk.gray(
							`Directory exists and contains ${files.length} item(s)`,
						) +
							(hasBmadInstall
								? chalk.yellow(
										` including existing BMAD installation (${path.basename(bmadDir)})`,
									)
								: ""),
					);
				} else {
		...
		if (expanded.startsWith("~")) {
			if (expanded === "~") {
				...
				const username =
					separatorIndex === -1
						? restOfPath
						: restOfPath.slice(0, separatorIndex);
				if (username) {
					throw new Error(
	...
	async promptAgentVibes(projectDir) {
		CLIUtils.displaySection(
			"🎤 Voice Features",
			"Enable TTS for multi-agent conversations",
		);
		...
		const answers = await inquirer.prompt([
			{
				...
				name: "enableTts",
				message:
					"Enable Agents to Speak Out loud (powered by Agent Vibes? Claude Code only currently)",
	...
	async checkAgentVibesInstalled(projectDir) {
		...
		return (
			(await fs.pathExists(hookPath)) && (await fs.pathExists(playTtsPath))
		);
	}
}
