...
class WebBundler {
	...
	async bundleTeam(moduleName, teamFile) {
		...
		const hasPartyFile = teamConfig.party?.endsWith(".csv");
		if (hasPartyFile) {
	 ...
	 */
	buildTeamBundle(_teamMetadata, agentXmls, dependencies) {
		const parts = [
		...
		if (dependencies && dependencies.size > 0) {
			...
			for (const [_id, content] of dependencies) {
	...
	async processWorkflowDependency(
		...
		if (workflowConfig.web_bundle?.use_advanced_elicitation) {
		...
		if (workflowConfig.web_bundle?.web_bundle_files) {
	...
	displaySummary() {
		...
		const hasActualWarnings = this.stats.warnings.some(
			(w) => w?.warnings && w.warnings.length > 0,
		);
		...
		if (hasActualWarnings) {
			...
			for (const agentWarning of this.stats.warnings) {
				if (agentWarning?.warnings && agentWarning.warnings.length > 0) {
					console.log(chalk.bold(`\n  ${agentWarning.agent}:`));
