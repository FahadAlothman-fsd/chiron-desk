...
const {
	DependencyResolver,
} = require("../installers/lib/core/dependency-resolver");
...
class WebBundler {
	...
	async bundleAgent(moduleName, agentFile, shouldTrack = true) {
		...
		const { dependencies, skippedWorkflows } =
		...
		const defaultPartyPath = path.join(
			this.modulesPath,
			moduleName,
			"teams",
			"default-party.csv",
		...
		if (await fs.pathExists(defaultPartyPath)) {
			...
			const processedPartyContent =
				this.processProjectRootReferences(partyContent);
			// Wrap as text to preserve raw CSV format in CDATA
			const wrappedParty = this.wrapContentInXml(
				processedPartyContent,
				"bmad/_cfg/agent-manifest.csv",
				"text",
			);
			dependencies.set("bmad/_cfg/agent-manifest.csv", wrappedParty);
			console.log(
				chalk.gray("    + Added party manifest from module default-party.csv"),
			);
		}
	...
	async bundleTeam(moduleName, teamFile) {
		...
		if (hasPartyFile) {
			...
			const partyPath = path.join(
				path.dirname(teamPath),
				teamConfig.party.replace(/^\.\//, ""),
			);
			if (await fs.pathExists(partyPath)) {
				...
				dependencies.set("bmad/_cfg/agent-manifest.csv", wrappedParty);
				console.log(
					chalk.gray(`    + Added agent manifest from: ${teamConfig.party}`),
				);
			} else {
		...
		if (await fs.pathExists(orchestratorXmlPath)) {
			...
			console.log(
				chalk.gray("    + Added orchestrator: bmad-web-orchestrator"),
			);
		} else {
			console.log(
				chalk.yellow(`    ⚠ Orchestrator not found at: ${orchestratorXmlPath}`),
			);
		}
		...
		if (
			...
			console.log(
				chalk.yellow("    ⚠ No agents specified in team configuration"),
			);
		}
		...
		for (const agentName of agentsToBundle) {
			...
			const isYaml = await fs.pathExists(agentPath);
			...
			if (isYaml) {
				...
				if (agentYaml?.agent?.webskip === true) {
					console.log(
						chalk.gray(`    ⊘ Skipped agent (webskip="true"): ${agentName}`),
					);
					continue;
				}

				// Build YAML agent in-memory - skip activation for team agents (orchestrator handles it)
				const xmlContent = await this.yamlBuilder.buildFromYaml(
					agentPath,
					null,
					{
			...
			if (this.shouldSkipBundling(agentXml)) {
				console.log(
					chalk.gray(`    ⊘ Skipped agent (bundle="false"): ${agentName}`),
				);
				continue;
			}
			...
			const agentWarnings = [];
			const { dependencies: agentDeps, skippedWorkflows } =
				await this.resolveAgentDependencies(
					agentXml,
					moduleName,
					agentWarnings,
			...
			// Remove commands for skipped workflows from agent XML
			if (skippedWorkflows.length > 0) {
				agentXml = this.removeSkippedWorkflowCommands(
					agentXml,
					skippedWorkflows,
				);
			}
		...
		const bundle = this.buildTeamBundle(
			teamConfig.bundle,
			allAgentXmls,
			dependencies,
		);
	...
	async vendorCrossModuleWorkflows(moduleName) {
		...
		for (const agentFile of yamlFiles) {
			...
			const workflowInstallItems = menuItems.filter(
			...
			for (const item of workflowInstallItems) {
				...
				const sourceMatch = sourceWorkflowPath.match(
					/\{project-root\}\/(?:\{bmad_folder\}|bmad)\/([^/]+)\/workflows\/(.+)/,
				);
				const installMatch = installWorkflowPath.match(
					/\{project-root\}\/(?:\{bmad_folder\}|bmad)\/([^/]+)\/workflows\/(.+)/,
				);
				...
				const actualSourceWorkflowPath = path.join(
					...
					sourceModule,
					"workflows",
					sourceWorkflowRelPath,
				);
				const actualDestWorkflowPath = path.join(
					this.modulesPath,
					installModule,
					"workflows",
					installWorkflowRelPath,
				);
				...
				if (!(await fs.pathExists(actualSourceWorkflowPath))) {
					console.log(
						chalk.yellow(
							`    ⚠ Source workflow not found for vendoring: ${sourceWorkflowPath}`,
						),
					);
					continue;
				}
				...
				if (await fs.pathExists(workflowYamlPath)) {
					await this.updateWorkflowConfigSource(
						workflowYamlPath,
						installModule,
					);
				}
				...
				console.log(
					chalk.dim(
						`    → Vendored workflow: ${sourceWorkflowRelPath} → ${installModule}/workflows/${installWorkflowRelPath}`,
					),
				);
			}
		}
	...
	async updateWorkflowConfigSource(workflowYamlPath, newModuleName) {
		...
		const configSourcePattern =
			/config_source:\s*["']?\{project-root\}\/(?:\{bmad_folder\}|bmad)\/[^/]+\/config\.yaml["']?/g;
		const newConfigSource = `config_source: "{project-root}/{bmad_folder}/${newModuleName}/config.yaml"`;

		const updatedYaml = yamlContent.replaceAll(
			configSourcePattern,
			newConfigSource,
		);
	...
	async preDiscoverModule(moduleName) {
		...
		this.discoveredAgents = this.discoveredAgents.filter(
	...
	extractFileReferences(xml) {
		...
		const xmlWithoutAgentId = xml.replace(
			/<agent[^>]*id="[^"]*"[^>]*>/,
			(match) => {
	...
	removeSkippedWorkflowCommands(agentXml, skippedWorkflows) {
		...
		for (const workflowPath of skippedWorkflows) {
			...
			const escapedPath = workflowPath.replaceAll(
				/[.*+?^${}()|[\]\\]/g,
				String.raw`\$&`,
			);
			...
			const itemWorkflowPattern = new RegExp(
				`\\s*<item\\s+[^>]*workflow="[^"]*${escapedPath}"[^>]*>.*?</item>\\s*`,
				"gs",
			);
			...
			const itemRunWorkflowPattern = new RegExp(
				`\\s*<item\\s+[^>]*run-workflow="[^"]*${escapedPath}"[^>]*>.*?</item>\\s*`,
				"gs",
			);
			...
			const cPattern = new RegExp(
				`\\s*<c\\s+[^>]*run-workflow="[^"]*${escapedPath}"[^>]*>.*?</c>\\s*`,
				"gs",
			);
	...
	async processFileDependency(
		...
		if (
			filePath === "bmad/_cfg/agent-manifest.csv" ||
			filePath.endsWith("/agent-manifest.csv")
		) {
			return;
		}
		...
		if (filePath.includes("*")) {
			await this.processWildcardDependency(
				filePath,
				dependencies,
				processed,
				moduleName,
		...
		if (
			ext === ".xml" ||
			(ext === ".md" && processedContent.trim().startsWith("<"))
		) {
			fileType = "xml";
		} else
			switch (ext) {
		...
		const wrappedContent = this.wrapContentInXml(
			processedContent,
			filePath,
			fileType,
		);
	...
	async processWorkflowDependency(
		...
		if (
			workflowConfig.web_bundle &&
			typeof workflowConfig.web_bundle === "object"
		) {
		...
		const wrappedYaml = this.wrapContentInXml(
			bundleYamlContent,
			yamlId,
			"yaml",
		);
		...
		await this.includeCoreWorkflowFiles(
			dependencies,
			processed,
			moduleName,
			warnings,
		...
		if (
			workflowConfig.web_bundle &&
			workflowConfig.web_bundle.use_advanced_elicitation
		) {
			await this.includeAdvancedElicitationFiles(
				dependencies,
				processed,
				moduleName,
				warnings,
		...
		if (
			workflowConfig.web_bundle &&
			workflowConfig.web_bundle.web_bundle_files
		) {
			...
			for (const bundleFilePath of bundleFiles) {
				...
				const bundleActualPath = this.resolveFilePath(
					bundleFilePath,
					moduleName,
				);
	 ...
	 */
	async includeCoreWorkflowFiles(
		dependencies,
		processed,
		moduleName,
		warnings = [],
	 ...
	 */
	async includeAdvancedElicitationFiles(
		dependencies,
		processed,
		moduleName,
		warnings = [],
	 ...
	 */
	async processWildcardDependency(
		pattern,
		dependencies,
		processed,
		moduleName,
	...
	injectHelpExitMenuItems(agentXml) {
		...
		const hasHelp =
			agentXml.includes('cmd="*help"') || agentXml.includes('trigger="*help"');
		const hasExit =
			agentXml.includes('cmd="*exit"') || agentXml.includes('trigger="*exit"');
		...
		if (!hasExit) {
			menuItems.push(
				`${indent}<item cmd="*exit">Exit with confirmation</item>`,
			);
		}
	...
	injectWebActivation(agentXml) {
		...
		if (!fs.existsSync(activationPath)) {
			console.warn(
				chalk.yellow(
					"Warning: agent-activation-web.xml not found, skipping activation injection",
				),
			);
			return agentXml;
		}
		...
		if (!closingTagMatch) {
			console.warn(
				chalk.yellow(
					"Warning: Could not find </agent> tag for activation injection",
				),
			);
			return agentXml;
		}
	...
	async createTempManifests() {
		...
		const agentPartyPath = path.join(
			this.tempManifestDir,
			"agent-manifest.csv",
		);
		await AgentPartyGenerator.writeAgentParty(
			agentPartyPath,
			this.discoveredAgents,
			{ forWeb: true },
		);
		...
		for (let i = 0; i < parts.length; i++) {
			...
			if (part.startsWith("<?xml")) {
				...
				if (
					...
					result +=
						TAB.repeat(depth) + part + nextPart.trim() + parts[i + 2] + "\n";
	...
	displaySummary() {
		...
		console.log(
			`  Successfully bundled:  ${chalk.green(this.stats.bundledAgents)}`,
		);
		if (this.stats.skippedAgents > 0) {
			console.log(
				`  Skipped (webskip/bundle): ${chalk.gray(this.stats.skippedAgents)}`,
			);
		}

		if (this.stats.failedAgents > 0) {
			console.log(
				`  Failed to bundle:      ${chalk.red(this.stats.failedAgents)}`,
			);
		}

		if (this.stats.invalidXml > 0) {
			console.log(
				`  Invalid XML bundles:   ${chalk.yellow(this.stats.invalidXml)}`,
			);
		}

		// Display warnings summary
		// Check if there are actually any warnings with content
		const hasActualWarnings = this.stats.warnings.some(
			(w) => w && w.warnings && w.warnings.length > 0,
		);
		...
		if (hasActualWarnings) {
			...
			for (const agentWarning of this.stats.warnings) {
				if (
					agentWarning &&
					agentWarning.warnings &&
					agentWarning.warnings.length > 0
				) {
					console.log(chalk.bold(`\n  ${agentWarning.agent}:`));
