...
function parseTestMetadata(filePath) {
	...
	for (const line of lines) {
		...
		if (keysMatch) {
			errorExpectation.keys = keysMatch[1]
				.split(",")
	...
	return {
		shouldPass,
		errorExpectation:
			Object.keys(errorExpectation).length > 0 ? errorExpectation : null,
		pathContext,
	};
}
...
function validateError(error, expectation) {
	...
	if (expectation.code && error.code !== expectation.code) {
		return {
			valid: false,
			reason: `Expected code "${expectation.code}", got "${error.code}"`,
		};
	}
	...
	if (
		expectation.code === "custom" &&
		expectation.message &&
		error.message !== expectation.message
	) {
		return {
			valid: false,
	...
	if (
		...
		error.minimum !== expectation.minimum
	) {
		return {
			valid: false,
			reason: `Expected minimum ${expectation.minimum}, got ${error.minimum}`,
		};
	}
	...
	if (expectation.expected && error.expected !== expectation.expected) {
		return {
			valid: false,
			reason: `Expected type "${expectation.expected}", got "${error.expected}"`,
		};
	}
	...
	if (expectation.received && error.received !== expectation.received) {
		return {
			valid: false,
			reason: `Expected received "${expectation.received}", got "${error.received}"`,
		};
	}
	...
	try {
		...
		if (!validationPath) {
			...
			const relativePath = path.relative(
				path.join(__dirname, "fixtures/agent-schema"),
				filePath,
			);
...
async function main() {
	...
	console.log(
		`${colors.cyan}║  Agent Schema Validation Test Suite                      ║${colors.reset}`,
	);
	console.log(
		`${colors.cyan}╚═══════════════════════════════════════════════════════════╝${colors.reset}\n`,
	);
	...
	console.log(
		`Found ${colors.cyan}${testFiles.length}${colors.reset} test fixture(s)\n`,
	);
	...
	for (const [categoryKey, files] of Object.entries(categories).sort()) {
		...
		console.log(
			`${colors.blue}${validLabel} ${categoryLabel} (${validInvalid})${colors.reset}`,
		);
		...
		for (const testFile of files) {
			...
			if (result.passed) {
				passedTests++;
				console.log(
					`  ${colors.green}✓${colors.reset} ${testName} ${colors.dim}${result.message}${colors.reset}`,
				);
			} else {
				console.log(
					`  ${colors.red}✗${colors.reset} ${testName} ${colors.red}${result.message}${colors.reset}`,
				);
				failures.push({
	...
	console.log(
		`  Failed: ${passedTests === totalTests ? colors.green : colors.red}${totalTests - passedTests}${colors.reset}`,
	);
	console.log(
		`${colors.cyan}═══════════════════════════════════════════════════════════${colors.reset}\n`,
	);
