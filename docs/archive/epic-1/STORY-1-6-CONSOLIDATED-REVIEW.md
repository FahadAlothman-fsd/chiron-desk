...
| :------------------------------ | :---------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------- |
| **AC6: Path Selection Cards**   | Custom React component for workflow paths | **Generic `OptionCard` System** that handles _any_ selection (Complexity, Paths, Future Tools) via JSON schema (`displayConfig`).                              | **Better** (Reusable) |
...
**The "Drift":** We stopped registering tools _at all_ until variables exist.
**Why:**

- The AI would try to call `select_workflow_path` before `complexity` was set, causing crashes.
- **Result:** The system is self-healing. The agent literally _cannot_ call a tool out of order.
