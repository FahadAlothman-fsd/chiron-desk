...
For this documentation pass, `WU.SETUP` is the slice we are converging toward and documenting as the intended next seed shape.

- Work unit scope: `WU.SETUP`
- Workflow scope: `setup-project`
...
3. Map the slice into Chiron structures:
   - work units
   - transitions
   - artifacts and slots
   - workflows
   - steps
   - agents
4. Add only that mapped slice to the active seed.
5. Validate the slice against canonical tables and current architecture docs.
