#!/usr/bin/env python3
"""
Script to analyze OpenCode sessions for the chiron project and categorize them by 2-week periods.
"""
...
def parse_session_file(filepath):
    """Parse a session file and extract relevant information."""
    try:
        ...
        # Extract agents from messages (looking for agent mentions in messages)
        agents_used = set()
        main_work_area = "Unknown"
        
        for msg in messages[:5]:  # Look at first 5 messages to determine work area
            if isinstance(msg, dict):
                content = msg.get('content', '')
                if isinstance(content, str):
                    # Look for agent mentions
                    agent_patterns = [
                        r'\b(bmad-master|dev|sm|architect|analyst|Sisyphus|frontend|backend|database)\b',
                        r'\@(bmad-master|dev|sm|architect|analyst|Sisyphus|frontend|backend|database)',
                        r'agent:\s*(bmad-master|dev|sm|architect|analyst|Sisyphus|frontend|backend|database)'
                    ]
                    for pattern in agent_patterns:
                        matches = re.findall(pattern, content, re.IGNORECASE)
                        agents_used.update([m.lower() for m in matches])
        ...
        # Determine story/epic from title and first messages
...
def main():
    ...
    print(f"Analyzing OpenCode sessions from: {opencode_storage}")
    
    # Find all session files
    session_files = []
    for session_dir in opencode_storage.iterdir():
    ...
    for session in chiron_sessions:
        # Try to determine sprint from updated_at, then created_at, then filename timestamp
        sprint = None
    ...
    for sprint_name in sprint_order:
        if sprint_name in sprint_data:
            ...
            report += f"- Sessions: {len(data['sessions'])} ({', '.join(session_ids[:10])}"
            if len(session_ids) > 10:
                report += f" ... and {len(session_ids) - 10} more"
            report += ")\n"
