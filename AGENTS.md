# AGENTS.md - Chiron Monorepo

## Build/Lint/Test Commands
- Build: `turbo build`
- Lint: `turbo lint`
- Type check: `turbo check-types`
- Dev: `turbo dev` (or specific: `turbo -F web dev`)
- No explicit test scripts; use `turbo test` if configured, or run Jest/Vitest in specific apps for single tests.

## Code Style Guidelines
- **Language**: TypeScript for all code; strict mode enabled.
- **Imports**: Use absolute paths with `@/` alias (e.g., `@/app/global.css`); prefer named imports.
- **Formatting**: Single quotes, semicolons, 2-space indentation; use Prettier.
- **Naming**: PascalCase for components/interfaces, camelCase for functions/variables; descriptive names.
- **Types**: Explicit types; avoid `any`; use interfaces for objects.
- **Error Handling**: Use try-catch; throw custom errors with messages; handle async errors with `.catch()`.
- **Components**: Functional components with hooks; props destructuring.
- **Linting**: Follow ESLint rules; no console.logs in production.
