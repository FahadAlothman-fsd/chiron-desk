...
export function streamTextWithRetry(
	...
	return streamFn().pipe(
		...
		Effect.tapError((error) =>
			Effect.logError(`Stream failed after retries: ${error.cause}`),
		),
	);
}
					...
					if (tokenCount - lastCheckpoint >= checkpointInterval) {
						yield* Effect.tryPromise({
							try: () =>
								...
								new StreamingError({ cause: error, phase: "emit" }),
						});
	...
	return Effect.gen(function* () {
		...
		yield* Effect.tryPromise({
			try: () =>
				...
				new ChatServiceError({ cause: error, operation: "add", sessionId }),
		});
	});
}
	...
	return Effect.gen(function* () {
		...
		yield* Effect.addFinalizer(() =>
			Effect.gen(function* () {
				...
				yield* Effect.log(
					`Stream interrupted, saved ${accumulated.length} chars`,
				);
			}),
		);
