	...
	Effect.gen(function* () {
		...
		const get = (
			...
			Effect.tryPromise({
				try: async () => {
					...
					new VariableDatabaseError({ operation: "get", cause: error }),
			});
		...
		const getAll = (
			...
			Effect.tryPromise({
				try: async () =>
					...
					new VariableDatabaseError({ operation: "getAll", cause: error }),
			});
			...
			Effect.gen(function* () {
				const result = yield* Effect.tryPromise({
					try: async () => {
						...
						new VariableDatabaseError({ operation: "set", cause: error }),
				});
		...
		const merge = (
			...
			Effect.all(
				Object.entries(vars).map(([name, value]) =>
					set(executionId, name, value, source),
				),
		...
		const deleteVar = (
			...
			Effect.tryPromise({
				try: async () => {
					...
					new VariableDatabaseError({ operation: "delete", cause: error }),
			});
		...
		> =>
			Effect.gen(function* () {
				...
				const sortedVars = [...allVars].sort((a, b) => {
		...
		> =>
			Effect.gen(function* () {
				...
				if (!childExecution) {
					return yield* Effect.fail(
						new ExecutionNotFoundError({ executionId: childExecutionId }),
					);
				}
