...
function asyncIterableToStream<A>(iterable: AsyncIterable<A>): Stream.Stream<A, StreamingError> {
...
function loadModelFromConfig(config: ModelConfig & { apiKey: string }): LanguageModel {
  switch (config.provider) {
    ...
    default:
      throw new Error(`Unsupported model provider: ${(config as ModelConfig).provider}`);
  }
}
...
function parseModelConfigString(modelString?: string | null): ModelConfig {
  ...
  if (parts.length !== 2) {
    console.warn(`[AIProvider] Invalid model format: ${modelString}, using default`);
    return getDefaultModelConfig();
  }

  const [provider, modelId] = parts;
  const validProviders: AIProvider[] = ["openrouter", "opencode", "openai", "anthropic"];

  if (!validProviders.includes(provider as AIProvider)) {
    console.warn(`[AIProvider] Unsupported provider: ${provider}, using default`);
    return getDefaultModelConfig();
  }
        ...
        Effect.tryPromise({
          try: async () => {
            ...
            const streamResult: StreamResult = {
              ...
              fullStream: asyncIterableToStream(result.fullStream as AsyncIterable<TextStreamPart>),
