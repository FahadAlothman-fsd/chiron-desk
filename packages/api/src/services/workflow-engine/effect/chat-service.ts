...
export interface ChatService {
	...
	getSession: (
		sessionId: string,
	) => Effect.Effect<ChatSession, SessionNotFoundError>;
	...
	updateSession: (
		sessionId: string,
		update: Partial<
			Pick<ChatSession, "status" | "title" | "completedAt" | "metadata">
		>,
		...
		const getNextSequenceNum = (sessionId: string) =>
			Effect.tryPromise({
				try: async () => {
					...
					new ChatServiceError({ cause: error, operation: "get", sessionId }),
			});
			...
			createSession: (executionId, stepId, metadata) =>
				Effect.tryPromise({
					try: async () => {
						...
						new ChatServiceError({ cause: error, operation: "create" }),
				}),
