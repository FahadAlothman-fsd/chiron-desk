...
const mockVariableService = {
	...
	get: (executionId: string, name: string) =>
		Effect.succeed(
			name === "artifact:test-artifact" ? { content: "test" } : null,
		),
	set: (
		executionId: string,
		name: string,
...
describe("ToolBuilder", () => {
	...
	describe("buildToolsFromConfig", () => {
		it("should build tools from default configs", async () => {
			...
			const result = await Effect.runPromise(
				buildToolsFromConfig(DEFAULT_TOOL_CONFIGS, context),
			);
		...
		it("should build update-variable tool", async () => {
			...
			const result = await Effect.runPromise(
				buildToolsFromConfig(configs, context),
			);
	...
	describe("executeTool", () => {
		...
		it("should return error for unknown tool type", async () => {
			...
			const result = await Effect.runPromise(
				executeTool("unknown", "unknown" as any, {}, context),
			);
	...
	describe("DEFAULT_TOOL_CONFIGS", () => {
		it("should have correct risk levels", () => {
			const updateVar = DEFAULT_TOOL_CONFIGS.find(
				(c) => c.name === "update_variable",
			);
			const snapshot = DEFAULT_TOOL_CONFIGS.find(
				(c) => c.name === "snapshot_artifact",
			);
