...
describe("error-utils", () => {
  describe("withRetry", () => {
    ...
    test("retries on failure then succeeds", async () => {
      ...
      const result = await Effect.runPromise(withRetry(effect, { baseDelayMs: 10 }));
      expect(result).toBe("success");
  ...
  describe("withTimeout", () => {
    test("succeeds within timeout", async () => {
      const effect = Effect.succeed("success");
      const result = await Effect.runPromise(withTimeout(effect, 1000, "step-1"));
      expect(result).toBe("success");
    });
    ...
    test("fails with StepTimeoutError when exceeded", async () => {
      ...
      const exit = await Effect.runPromiseExit(withTimeout(effect, 10, "step-1"));
