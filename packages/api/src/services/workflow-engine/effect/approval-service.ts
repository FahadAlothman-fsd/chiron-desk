...
import type { ApprovalMode, RiskLevel, ToolApprovalConfig } from "./tool-builder";
...
export interface ApprovalService {
  ...
  getUserSettings: (userId: string) => Effect.Effect<UserAutoApproveSettings, ApprovalServiceError>;
  ...
  getSessionState: (executionId: string) => Effect.Effect<SessionApprovalState, never>;
    ...
    return {
      ...
      shouldAutoApprove: (request, userSettings, sessionState) =>
        Effect.sync(() => {
          ...
          if (sessionState.approveAllUntil && new Date() < sessionState.approveAllUntil) {
            return {
              ...baseDecision,
              autoApprove: true,
      ...
      logDecision: (entry) =>
        Effect.tryPromise({
          try: async () => {
            ...
            const [result] = await db.insert(approvalAudit).values(newEntry).returning();
          ...
          catch: (error) => new ApprovalServiceError({ cause: error, operation: "log" }),
        }),
      ...
      getSessionState: (executionId) => Effect.sync(() => getSessionState(executionId)),
