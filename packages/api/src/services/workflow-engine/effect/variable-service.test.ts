...
afterAll(async () => {
  await db.delete(workflowExecutions).where(eq(workflowExecutions.id, testExecutionId));
  await db.delete(workflowExecutions).where(eq(workflowExecutions.id, parentExecutionId));
  ...
  describe("CRUD operations", () => {
    test("set creates a new variable", async () => {
      const program = Effect.gen(function* () {
        ...
        const variable = yield* service.set(testExecutionId, "test-var", "test-value", "input");
    ...
    test("set updates existing variable and records history", async () => {
      const program = Effect.gen(function* () {
        ...
        yield* service.set(testExecutionId, "history-var", "updated", "step", 1);

        const variable = yield* service.get(testExecutionId, "history-var");
        expect(variable?.value).toBe("updated");

        const history = yield* service.getHistory(testExecutionId, "history-var");
  ...
  describe("Template resolution", () => {
    test("resolveTemplate substitutes variables", async () => {
      const program = Effect.gen(function* () {
        ...
        const result = yield* service.resolveTemplate("Hello, {{name}}!", testExecutionId);
    ...
    test("resolveTemplate fails for missing variables", async () => {
      const program = Effect.gen(function* () {
        const service = yield* VariableService;
        return yield* service.resolveTemplate("Hello, {{missing}}!", testExecutionId);
      });
  ...
  describe("Parent-child propagation", () => {
    ...
    test("propagateToParent records history with correct source", async () => {
      const program = Effect.gen(function* () {
        ...
        const history = yield* service.getHistory(parentExecutionId, "child-output");
        const propagatedEntry = history.find((h) => h.source === "child-propagation");
  ...
  describe("4-level precedence (AC4)", () => {
    test("system variables override step variables in template resolution", async () => {
      const program = Effect.gen(function* () {
        ...
        yield* service.set(uniqueExecId, "priority-var", "system-value", "system");
