...
describe("Error Recovery", () => {
  ...
  describe("executeToolWithTimeout", () => {
    it("should return result if completes before timeout", async () => {
      ...
      const result = await Effect.runPromise(executeToolWithTimeout(effect, 1000, "test-tool"));

      expect(result).toEqual({ success: true, value: 42 });
    });

    it("should return timeout error if exceeds timeout", async () => {
      const slowEffect = Effect.delay(Effect.succeed("done"), Duration.seconds(5));

      const result = await Effect.runPromise(executeToolWithTimeout(slowEffect, 50, "slow-tool"));
