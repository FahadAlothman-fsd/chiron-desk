...
export class StateManager {
  ...
  async updateExecutedSteps(
    ...
    const executedSteps = (execution.executedSteps as Record<number, ExecutedStepData>) || {};
  ...
  async mergeExecutionVariables(
    ...
    const currentVariables = (execution.variables as Record<string, unknown>) || {};
   ...
   */
  async failExecution(executionId: string, error: string, errorStep?: number): Promise<void> {
    await db
      .update(workflowExecutions)
      .set({
        status: "failed",
