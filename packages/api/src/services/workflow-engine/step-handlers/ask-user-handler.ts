...
export class AskUserStepHandler implements StepHandler {
  async executeStep(
    ...
    if (userInput === undefined || userInput === null) {
      console.log("[AskUserHandler] No user input - returning requiresUserInput: true");
      return {
        output: {},
    ...
    console.log("[AskUserHandler] User input provided - validating and continuing");
   ...
   * Validate user input based on response type
   */
  private async validateInput(userInput: unknown, config: AskUserStepConfig): Promise<unknown> {
    switch (config.responseType) {
      ...
      default:
        throw new Error(`Unsupported response type: ${(config as any).responseType}`);
    }
  }
   ...
   * Validate path input
   */
  private async validatePath(inputPath: string, config: AskUserStepConfig): Promise<string> {
    // Check required
  ...
  private validateString(input: string, config: AskUserStepConfig): string {
    ...
    // Check min length
    if (config.validation?.minLength && input.length < config.validation.minLength) {
      throw new ValidationError(`Input must be at least ${config.validation.minLength} characters`);
    }

    // Check max length
    if (config.validation?.maxLength && input.length > config.validation.maxLength) {
      throw new ValidationError(`Input must be at most ${config.validation.maxLength} characters`);
    }
  ...
  private validateNumber(input: unknown, config: AskUserStepConfig): number {
    ...
    if (config.validation?.min !== undefined && num < config.validation.min) {
      throw new ValidationError(`Number must be at least ${config.validation.min}`);
    }

    // Check max
    if (config.validation?.max !== undefined && num > config.validation.max) {
      throw new ValidationError(`Number must be at most ${config.validation.max}`);
    }
