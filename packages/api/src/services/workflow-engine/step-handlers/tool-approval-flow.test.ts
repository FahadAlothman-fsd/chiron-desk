...
When the user describes their project, call update_summary to generate a project description.`,
	...
	describe("Tool Execution → Approval State Flow", () => {
		it("should save tool result to approval_states when requiresApproval=true", async () => {
			...
			testContext.executionVariables.mastra_thread_id =
				initResult.output.mastra_thread_id;
			...
			testContext.executionVariables.approval_states = {
				update_summary: {
					...
					reasoning:
						"User wants team collaboration features, task organization is core goal",
					rejection_history: [],
			...
			const approvalStates = testContext.executionVariables
				.approval_states as Record<string, any>;
	...
	describe("Approval State Data Structure", () => {
		it("should preserve complete approval state structure", async () => {
			const mockApprovalState = {
				...
				reasoning:
					"User emphasized personalization and product discovery features",
				rejection_history: [],
			...
			expect(approvalStates.update_summary.reasoning).toContain(
				"personalization",
			);
