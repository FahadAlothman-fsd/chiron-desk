...
export class AskUserChatStepHandler implements StepHandler {
	...
	private async buildToolsForAgent(
		...
		if (config.tools && config.tools.length > 0) {
			...
			for (const toolConfig of config.tools) {
				...
				// Get approval states to check for variables generated by previous tools in this step
				const approvalStates =
					(context.executionVariables.approval_states as Record<string, any>) ||
					{};

				const missingVars = requiredVars.filter((varName) => {
					// 1. Check in global execution variables
					if (varName in context.executionVariables) return false;

					// 2. Check in approved tool outputs (for intra-step dependencies)
					// We only care about "approved" tools because "pending" ones haven't been confirmed by user
	...
	async executeStep(
		...
		// Only include guidance for tools that are:
		// 1. Registered (prerequisites met, so they exist in the tools object)
		// 2. Not yet approved (no need to guide agent on completed tasks)
		...
		const toolsGuidance = (config.tools || [])
			.filter((t) => {
				...
				if (!registeredToolNames.includes(t.name)) return false;
