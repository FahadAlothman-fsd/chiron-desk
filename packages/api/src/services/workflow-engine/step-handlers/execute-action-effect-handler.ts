...
export const ExecuteActionHandler = Context.GenericTag<ExecuteActionHandler>(
		...
		Effect.tryPromise({
			try: async () => {
				...
				return {
					...
					preview: (result.output as Record<string, unknown>)?.preview as
						| boolean
						| undefined,
				};
			},
	...
	return {
		async executeStep(
			...
			const program = Effect.provide(
				Effect.flatMap(ExecuteActionHandler, (handler) =>
