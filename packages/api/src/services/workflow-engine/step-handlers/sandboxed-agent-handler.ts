...
export const SandboxedAgentHandlerLive = Layer.succeed(SandboxedAgentHandler, {
  ...
  execute: (input: StepHandlerInput, userInput?: unknown) =>
    Effect.sync(() => {
      ...
      const conversationState = (input.stepConfig as Record<string, unknown>)._conversationState as
        | {
      ...
      const approvedTools = new Set<string>(conversationState?.approvedTools ?? []);
