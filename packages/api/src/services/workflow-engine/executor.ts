...
export async function executeWorkflow(params: {
  ...
  try {
    ...
    workflowEventBus.emitWorkflowStarted(executionId, params.workflowId, params.userId);
...
export async function continueExecution(
  ...
  if (waitingStepNumbers.length > 0) {
    ...
    console.log(`[Executor] Found waiting step ${currentStepNumber}, resuming with user input`);
    ...
    console.log("[Executor] No completed or waiting steps, starting from step 1");
  }
  ...
  while (currentStepNumber !== null) {
    ...
    try {
      ...
      console.log(`[Executor] Step ${currentStep.stepNumber} (${currentStep.stepType}) result:`, {
      ...
      await mergeExecutionVariables(executionId, result.output);
      console.log(`[Executor] ✓ Merge completed for step ${currentStep.stepNumber}`);

      // Emit step_completed event
      workflowEventBus.emitStepCompleted(executionId, currentStep.stepNumber);

      console.log(`[Executor] Step ${currentStep.stepNumber} completed - continuing to next step`);
...
async function pauseExecution(executionId: string, stepNumber: number): Promise<void> {
  console.log("[Executor] pauseExecution called for:", executionId, "at step:", stepNumber);
  await db
    .update(workflowExecutions)
    .set({
      status: "paused",
