...
function validateWorkflowStructure(steps: WorkflowStep[]): void {
	...
	const sortedNumbers = Array.from(stepNumberSet).sort((a, b) => a - b);
	for (let i = 0; i < sortedNumbers.length - 1; i++) {
		if (sortedNumbers[i + 1] - sortedNumbers[i] > 1) {
			warnings.push(
				`Gap in step numbers: ${sortedNumbers[i]} -> ${sortedNumbers[i + 1]}`,
			);
		}
	}
	...
	if (cycles.length > 0) {
		...
		warnings.push(
			`Cycle detected (OK for state machines): ${cycleDescriptions}`,
		);
	}
