...
describe("Projects Router - Story 1.5", () => {
	...
	beforeAll(async () => {
		...
		if (!testUser) {
			throw new Error(
				"Test user not found. Run: bun run db:seed to create test user",
			);
		}
		...
		if (!workflow) {
			throw new Error(
				"No workflow initializer found - run workflow seed first",
			);
		}
		...
		await db
			.delete(projects)
			.where(eq(projects.name, "Test Project Story 1.5"));
	...
	test("workflow execution should be created for the project", async () => {
		const execution = await db.query.workflowExecutions.findFirst({
			where: (workflowExecutions, { eq }) =>
				eq(workflowExecutions.id, testExecutionId),
		});
	...
	test("projects.delete should remove project and cascade to executions", async () => {
		...
		const executionBefore = await db.query.workflowExecutions.findFirst({
			where: (workflowExecutions, { eq }) =>
				eq(workflowExecutions.projectId, testProjectId),
		});
