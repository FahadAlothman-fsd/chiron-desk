...
import { extractValues, getValueByPath, hasPath, setValueByPath } from "./json-path";
...
describe("getValueByPath", () => {
  ...
  it("should extract deeply nested value (3 levels)", () => {
    expect(getValueByPath(testObj, "tags.complexity.name")).toBe("Quick Flow Track");
  });
...
describe("Real-world usage scenarios", () => {
  ...
  it("should extract complexity option fields for simple card", () => {
    ...
    expect(displayDescription).toBe("Fast implementation track using tech-spec planning only");
  });
  ...
  it("should handle missing optional fields gracefully", () => {
    ...
    const subtitle = getValueByPath(option, "tags.recommendedFor.value", "No recommendation");
