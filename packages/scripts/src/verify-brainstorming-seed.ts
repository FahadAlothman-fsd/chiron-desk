...
async function verifyBrainstormingSeed() {
	...
	for (const step of steps) {
		...
		if (step.stepType === "ask-user-chat") {
			...
			console.log(
				`   Initial Message: ${config.initialMessage?.substring(0, 80)}...`,
			);
			...
			if (config.tools) {
				for (const tool of config.tools) {
					...
					if (tool.requiredVariables?.length > 0) {
						console.log(
							`        → Required Variables: ${tool.requiredVariables.join(", ")}`,
						);
					}
					if (tool.valueSchema) {
			...
			console.log(
				`   Completion Condition: ${config.completionCondition?.type}`,
			);
