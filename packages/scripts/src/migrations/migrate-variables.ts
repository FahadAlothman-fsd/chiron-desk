import { and, db, eq, variableHistory, variables, workflowExecutions } from "@chiron/db";

type VariableSource = "input" | "step" | "system" | "parent" | "child-propagation" | "migration";
    ...
    try {
      ...
      for (const [name, value] of Object.entries(varsJson)) {
        const existing = await db
          .select()
          .from(variables)
          .where(and(eq(variables.executionId, execution.id), eq(variables.name, name)))
          .limit(1);
