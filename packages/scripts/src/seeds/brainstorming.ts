...
export async function seedBrainstorming() {
	...
	if (!analystAgent) {
		console.error(
			"  ❌ Analyst agent not found - cannot seed brainstorming workflow",
		);
		return;
	}
	...
	if (!workflow) {
		console.error(
			"  ❌ Brainstorming workflow not found - run seedBrainstormingWorkflow first",
		);
		return;
	}
...
*Session configured and ready to begin*`;
	...
	if (existingSteps.length > 0) {
		console.log(
			"  ℹ️  Brainstorming steps already exist, skipping step seeding",
		);
		return;
	}
		...
		tools: [
			// Tool 1: Update Topic
			{
				...
				valueSchema: {
					type: "string",
					description:
						"A clear, concise session topic (let agent decide appropriate length)",
				},
			},
			...
			// Tool 2: Update Goals
			{
				...
				valueSchema: {
					...
					description:
						"List of session goals (agent determines appropriate quantity)",
				},
			},
					...
					output: [
						{
							...
							description:
								"Selected technique IDs (agent recommends appropriate number)",
							internal: false,
						},
						{
							name: "reasoning",
