...
export async function seedSixThinkingHatsTechnique() {
  ...
  if (!analystAgent) {
    console.error("  ❌ Analyst agent not found - cannot seed Six Thinking Hats workflow");
    return;
  }
  ...
  if (!workflow) {
    console.error("  ❌ Six Thinking Hats workflow not found - run workflow seeder first");
    return;
  }
  ...
  if (existingSteps.length > 0) {
    console.log("  ℹ️  Six Thinking Hats steps already exist, skipping step seeding");
    return;
  }
...
Start enthusiastically: "🎩 Let's put on different thinking hats to see this from ALL angles! First up - the 🤍 WHITE HAT (facts only): What do we KNOW for certain about this topic?"`,
    ...
    tools: [
      ...
      // Tool 6: BLUE HAT (Synthesis)
      {
        ...
        valueSchema: {
          type: "string",
          description: "Overall synthesis and big picture understanding from all perspectives",
        },
      },
    ],
  ...
  console.log("  ✓ Step 1: 6 Sequential Hats (White-Red-Yellow-Black-Green-Blue)");
}
