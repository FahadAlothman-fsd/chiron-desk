...
const CHIRON_MCP_CONTEXT_OPERATING_GUIDANCE =
  "You are operating in Chiron with access to MCP tools. Available tools:\n- chiron_read_step_execution_snapshot: read the current step objective, instructions, completion expectations, read set, and write set\n- chiron_read_context_fact_schema: inspect a context fact schema, cardinality, and allowed actions before writing\n- chiron_read_context_fact_instances: inspect the current values of a context fact before deciding whether to create, update, or remove\n- chiron_create_context_fact_instance: create a new value when a fact is empty or needs an additional instance\n- chiron_update_context_fact_instance: revise an existing value when later clarification or synthesis changes it\n- chiron_remove_context_fact_instance and chiron_delete_context_fact_instance: remove stale or incorrect values only when they no longer belong in the final result\n\nStart by calling `chiron_read_step_execution_snapshot`. Then use `chiron_read_context_fact_schema` and `chiron_read_context_fact_instances` before writing values whose current state you need to understand. Use the write tools deliberately: create when empty, update when refining, and remove or delete only when a value should no longer remain. Only write the context facts and artifact references that this step owns.";
    ...
    for (const [index, step] of spec.steps.entries()) {
      ...
      if (step.type === "agent") {
        ...
        methodologyWorkflowAgentSteps.push({
          ...
          liveStreamCount: 1,
          bootstrapPromptNoReply: spec.workUnitKey === "setup",
