...
function ProjectDashboard() {
  ...
  const allWorkflowsCompleted = !nextWorkflow && (recommendedData?.completedCount ?? 0) > 0;
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {nextWorkflow ? (
        ...
        <Card className="border-green-500/20 bg-gradient-to-r from-green-500/5 to-green-500/10">
          ...
          <CardContent>
            <p className="text-muted-foreground">
              You've completed all workflows in your workflow path. Congratulations!
            </p>
          </CardContent>
        </Card>
      ) : (
        <Card className="border-muted bg-muted/20">
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-muted-foreground">No Workflows Available</CardTitle>
            </div>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground">
              ...
              {workflowPath && (
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{workflowPath.displayName}</span>
                  </div>
                  {workflowPath.description && (
                    <p className="pl-6 text-muted-foreground text-sm">{workflowPath.description}</p>
                  )}
                </div>
              )}
              <div className="space-y-1">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track your journey through the BMAD methodology</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {PHASES.map((phase) => (
...
function PhaseItem({
  ...
  const isCompleted = Number.parseInt(phase.id, 10) < Number.parseInt(currentPhaseId, 10);
  const isPending = Number.parseInt(phase.id, 10) > Number.parseInt(currentPhaseId, 10);
  ...
  const phaseFullyCompleted = totalInPhase > 0 && completedInPhase === totalInPhase;
      ...
      <CollapsibleTrigger asChild>
        ...
        >
          ...
          <div className="w-24 shrink-0">
            ...
            <p className="mt-1 text-right text-muted-foreground text-xs">{progressPercent}%</p>
          </div>
        ...
        <div className="ml-12 space-y-2 border-muted border-l-2 py-2 pl-4">
          {isLoading ? (
            ...
            workflows.map((workflow) => {
              ...
              const liveData = liveExecutions?.find((e) => e.id === baseExecution?.id);
