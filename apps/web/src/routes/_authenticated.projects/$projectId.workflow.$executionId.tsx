...
export const Route = createFileRoute("/_authenticated/projects/$projectId/workflow/$executionId")({
...
function WorkflowExecutionPage() {
  ...
  const [dialogChildExecutionId, setDialogChildExecutionId] = useState<string | null>(null);
      ...
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            {error instanceof Error ? error.message : "The workflow execution doesn't exist."}
          </p>
          <Button
            className="mt-4"
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            This workflow execution hasn't been started yet or has no steps configured.
          </p>
          <Button
            className="mt-4"
            onClick={() => navigate({ to: "/projects/$projectId", params: { projectId } })}
  ...
  const handleExecuteWorkflow = (workflowId: string) => {
    ...
    const existingChild = childMetadata?.find((child) => child.workflowId === workflowId);
  ...
  return (
    <div className="h-screen w-full p-4">
      ...
      {dialogChildExecutionId && childExecutionData && childExecutionData.currentStep && (
        <WorkflowLayoutRenderer
      ...
      {pendingWorkflowId &&
        (() => {
          ...
          return (
            <Dialog open={!!pendingWorkflowId} onOpenChange={() => setPendingWorkflowId(null)}>
              <DialogContent className="max-h-[80vh] max-w-2xl overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Review the workflow details and variables that will be passed
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {pendingWorkflowData && pendingWorkflowData.length > 0 ? (
                      <>
                        <p className="font-medium">
                          {pendingWorkflowData[0].displayName || pendingWorkflowData[0].name}
                        </p>
                    ...
                    <p className="mt-1 text-muted-foreground text-xs">ID: {pendingWorkflowId}</p>
                  </div>

                  <div>
                    <h4 className="mb-2 font-medium text-sm">Variables to be passed</h4>
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleConfirmStart} disabled={createChildWorkflow.isPending}>
