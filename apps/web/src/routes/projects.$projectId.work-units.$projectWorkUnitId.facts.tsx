...
function getPathValidationKind(validation: unknown): "file" | "directory" | null {
  ...
  return validation.pathKind === "directory" ? "directory" : "file";
}
...
function WorkUnitFactValuePresentation(props: {
  value: unknown;
...
function getPrimitiveCardValue(entry: unknown): { instanceId?: string; value: unknown } {
  if (!isRecord(entry)) {
    return { value: entry };
  }
  ...
  const instanceId =
    typeof entry.instanceId === "string"
      ? entry.instanceId
      : typeof entry.workUnitFactInstanceId === "string"
        ? entry.workUnitFactInstanceId
        : undefined;

  return {
    ...(instanceId ? { instanceId } : {}),
    value: "value" in entry ? entry.value : entry,
  };
}
                    ...
                    <CardContent className="space-y-3">
                      ...
                      {card.currentValues.length > 0 ? (
                        <ul className="space-y-1 text-xs">
                          {card.currentValues.map((entry, index) => {
                            const value = getPrimitiveCardValue(entry);
                            return (
                              <li
                                key={value.instanceId ?? `${card.factDefinitionId}-value-${index}`}
