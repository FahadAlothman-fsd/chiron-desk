...
export function SelectionWithCustomCard({
  ...
  const [selectedOption, setSelectedOption] = useState<string>(
    suggestions.find((s) => s.recommended)?.value || suggestions[0]?.value || "",
  );
  ...
  function validateCustomValue(value: string): boolean {
    ...
    if (validation) {
      if (validation.minLength && value.length < validation.minLength) {
        setCustomValueError(`Must be at least ${validation.minLength} characters`);
        return false;
      }

      if (validation.maxLength && value.length > validation.maxLength) {
        setCustomValueError(`Must be at most ${validation.maxLength} characters`);
        return false;
      }
    ...
    >
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-lg">
          ...
          {isApproved && <span className="font-normal text-green-600 text-sm">Selected ✓</span>}
        </CardTitle>
      </CardHeader>
            ...
            {suggestions.map((suggestion, _idx) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <RadioGroupItem value={suggestion.value} id={suggestion.value} className="mt-1" />
                  <div className="min-w-0 flex-1">
                    <Label
                      htmlFor={suggestion.value}
                      className="flex cursor-pointer items-center gap-2 font-semibold text-base"
                    >
                      <code className="font-mono">{suggestion.label || suggestion.value}</code>
                      {suggestion.recommended && <Sparkles className="h-4 w-4 text-yellow-500" />}
                    </Label>
                    {suggestion.reasoning && (
                      <p className="mt-1 text-muted-foreground text-sm">{suggestion.reasoning}</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="min-w-0 flex-1 space-y-3">
                  ...
                  <div className="space-y-2">
                    <Input
                      ...
                      disabled={isApproved || isLoading}
                      className={`font-mono ${
                        customValueError ? "border-red-500 focus-visible:ring-red-500" : ""
                      }`}
                      onClick={(e) => {
                    ...
                    {/* Validation Message */}
                    {customValueError && <p className="text-red-600 text-sm">{customValueError}</p>}
      ...
      {!isApproved && (
        <CardFooter>
          <Button onClick={handleApprove} disabled={isLoading || !canSubmit} className="w-full">
            {isLoading ? (
              <>
