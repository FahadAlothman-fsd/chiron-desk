    ...
    <div className={cn("space-y-2", depth > 0 && "ml-4")}>
      ...
      {isExpanded && (
        <div className="space-y-1.5">
          {data.map((item, index) => {
            // Extract item values
            const title = getValueByPath<string>(item, config.itemFields.title, "");
              ...
              <div key={`${title}-${index}`} className="space-y-1">
                ...
                <div className="flex items-start gap-2 text-sm">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {subtitle && (
                      <span className="ml-2 text-muted-foreground text-xs">{subtitle}</span>
                    )}
                  </div>
                </div>
                  ...
                  config.itemFields.childFields && (
                    <div className="ml-4 space-y-1">
                      {children.map((child, childIndex) => {
                        ...
                        const childIcon = config.itemFields.childFields?.icon || "•";
                        ...
                        return (
                          ...
                          >
                            <span className="shrink-0 text-muted-foreground">{childIcon}</span>
                            <span className="text-muted-foreground">{childTitle}</span>
                          </div>
                        );
                      })}
                    </div>
