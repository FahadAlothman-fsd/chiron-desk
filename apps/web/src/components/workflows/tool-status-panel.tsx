                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {status === "blocked" && missingPrereqs.length > 0 && (
                        <div className="mt-1 text-orange-600 text-xs dark:text-orange-400">
                          Waiting for: {missingPrereqs.map((v) => v.replace(/_/g, " ")).join(", ")}
                        </div>
                      )}
                ...
                <TooltipContent side="right" className="max-w-xs">
                  <div className="space-y-1">
                    ...
                    {tool.requiresApproval && (
                      <div className="text-muted-foreground text-xs">Requires user approval</div>
                    )}
                    {tool.requiredVariables && tool.requiredVariables.length > 0 && (
                      <div className="text-muted-foreground text-xs">
                        Prerequisites:{" "}
                        {tool.requiredVariables.map((v) => v.replace(/_/g, " ")).join(", ")}
                      </div>
                    )}
                  </div>
                </TooltipContent>
