    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-border border-b bg-background px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          {stepGoal && <h3 className="font-semibold text-foreground text-sm">{stepGoal}</h3>}
        </div>
      </div>

      {/* Step Content: Takes 100% of remaining space */}
      <div className="flex flex-1 items-center justify-center overflow-auto p-4">{children}</div>
    </div>
  );
}
