...
export function ArtifactPreview({ execution, workflow }: ArtifactPreviewProps) {
  ...
  switch (templateData.artifactType) {
    case "brainstorming":
      return <BrainstormingPreview execution={execution} templateData={templateData} />;
    ...
    default:
      return (
        <div className="space-y-4 p-6">
          <div>
            <h3 className="mb-2 font-semibold text-lg">{templateData.displayName}</h3>
          ...
          <div className="flex h-64 items-center justify-center rounded-lg border border-dashed">
            <p className="text-center text-muted-foreground">
              Preview not available for artifact type: {templateData.artifactType}
            </p>
          </div>
        </div>
      );
...
function BrainstormingPreview({ execution, templateData }: BrainstormingPreviewProps) {
  ...
  const getVariableDescription = (varName: string): string => {
    const varMeta = templateData?.templateVariables?.find((v: any) => v.name === varName);
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="space-y-6">
        ...
        {/* Goals Section */}
        <div>
          <h4 className="mb-3 font-semibold text-muted-foreground text-sm">Session Goals</h4>
        ...
        {/* Techniques Section */}
        <div>
          <h4 className="mb-2 font-semibold text-muted-foreground text-sm">Selected Techniques</h4>
          {techniques && techniques.length > 0 ? (
            <div className="space-y-2">
              {techniques.map((techniqueId: string, idx: number) => {
                ...
                const technique = techniqueOptions.find((t: any) => t.id === techniqueId);

                return (
                  <div key={idx} className="rounded-md border bg-card p-3 text-sm">
                    <div className="font-medium">{technique?.displayName || techniqueId}</div>
        ...
        <div className="mt-6 rounded-md border-blue-500 border-l-4 bg-blue-50/50 p-3 dark:bg-blue-500/5">
          <p className="text-blue-900 text-xs dark:text-blue-300">
            💡 <strong>Tip:</strong> Chat with the agent to define your topic, goals, and select
            brainstorming techniques. This preview will update in real-time as you progress.
          </p>
        </div>
      </div>
    </div>
...
function CapturedIdeasSection({ variables }: CapturedIdeasSectionProps) {
  const capturedIdeas = variables.captured_ideas as Record<string, any> | undefined;

  // If no captured ideas yet, show empty state
  if (!capturedIdeas || Object.keys(capturedIdeas).length === 0) {
    return (
      <div>
        <h4 className="mb-2 font-semibold text-muted-foreground text-sm">Captured Ideas</h4>
        <div className="space-y-2">
          <div className="animate-pulse rounded-md border border-yellow-300 border-dashed bg-yellow-50/50 p-3 text-sm text-yellow-700 dark:border-yellow-700 dark:bg-yellow-500/5 dark:text-yellow-400">
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {Object.entries(capturedIdeas).map(([workflowId, data]: [string, any]) => {
              ...
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-muted-foreground text-xs">
                    {outputItems.length} {outputItems.length === 1 ? "insight" : "ideas"}
                  </span>
                </div>
                {completedAt && (
                  <span className="text-muted-foreground text-xs">Completed {completedAt}</span>
                )}
              </div>
              ...
              {outputItems.length > 0 ? (
                <div className="space-y-2">
                  {outputItems.map((item: any, idx: number) => {
                    ...
                    const displayText =
                      item.text || item.idea || item.value || item.name || JSON.stringify(item);
                ...
                <p className="text-muted-foreground text-sm italic">No output from this workflow</p>
              )}
            </div>
          );
        })}
