...
export function ToolStatusSidebar({
  ...
  const approvedCount = tools.filter((t) => getToolStatus(t) === "approved").length;
  ...
  const { data: instructionsData } = trpc.agents.getResolvedInstructions.useQuery(
    { agentId, executionId },
    {
                  ...
                  <AccordionItem key={tool.name} value={tool.name} className="border-b-0">
                    ...
                    <AccordionContent className="px-4 pb-4">
                      ...
                      {(() => {
                        ...
                        if (Array.isArray(options) && options.length > 0 && status !== "approved") {
                          return (
                            <div className="mb-3 rounded-md border border-blue-200 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-950">
                      ...
                      {status === "blocked" && missingPrereqs.length > 0 && (
                        <div className="mb-3 rounded-md border border-orange-200 bg-orange-50 p-3 dark:border-orange-800 dark:bg-orange-950">
                          ...
                          <div className="text-orange-700 text-xs dark:text-orange-300">
                            Waiting for:{" "}
                            {missingPrereqs.map((v) => v.replace(/_/g, " ")).join(", ")}
                          </div>
                        </div>
                      )}
                        ...
                        approvalState.rejection_history.length > 0 && (
                          <div className="mb-3 space-y-2">
                            <div className="font-medium text-muted-foreground text-xs uppercase tracking-wider">
                              Rejection History ({approvalState.rejection_history.length})
                            </div>
                            {approvalState.rejection_history.map((rejection, idx) => {
                              ...
                              const isLatest = idx === approvalState.rejection_history?.length - 1;
                        ...
                        <div className="mt-2 rounded-md border border-green-200 bg-green-50 p-3 dark:border-green-800 dark:bg-green-950">
                          ...
                          <div className="space-y-2">
                            {typeof approvalState.value === "string" ? (
                              ...
                              Object.entries(approvalState.value).map(([key, value]) => {
