...
export function ApprovalCardSelector({
  ...
  const uniqueOptions = availableOptions.reduce((acc: Record<string, unknown>[], option) => {
    ...
    const exists = acc.some((o) => {
      const existingKey =
        (o as any).id ||
        (displayConfig ? getValueByPath(o, displayConfig.fields.value) : (o as any).value) ||
        JSON.stringify(o);
  ...
  const isMultiSelect = Object.values(generatedValue).some((val) => Array.isArray(val));
  ...
  const aiRecommendedValue = (() => {
    ...
    return Array.isArray(firstVal) ? (firstVal as string[]) : (firstVal as string);
  })();
  ...
  const [selectedValue, setSelectedValue] = useState<string | string[]>(aiRecommendedValue);
  ...
  async function handleApprove() {
    ...
    if (!outputFieldKey) {
      throw new Error("Could not find output field in generatedValue matching available options");
    }
  ...
  async function handleReject() {
    if (!feedback.trim()) {
      toast.error("Please provide feedback explaining why you're rejecting this.");
      return;
    }
    ...
    >
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-lg">
          <span>
            {isApproved && <Check className="mr-2 inline h-5 w-5 text-green-600" />}
            {isRejected && <X className="mr-2 inline h-5 w-5 text-red-600" />}
            {displayToolName}
          </span>
          {isApproved && <span className="font-normal text-green-600 text-sm">Approved ✓</span>}
          {isRejected && <span className="font-normal text-red-600 text-sm">Rejected</span>}
        </CardTitle>
        {createdAt && (
      ...
      <CardContent className="space-y-4">
        {/* Options Accordion when approved */}
        {isApproved && (
          <div className={isOptionsExpanded ? "pb-3" : ""}>
            <button
              type="button"
            ...
            >
              ...
              <span>Selected {isMultiSelect ? "Options" : "Option"}</span>
            </button>
          </div>
        )}
          ...
          (displayConfig ? (
            ...
            <div className="space-y-3">
              {uniqueOptions.map((option, index) => {
                const optionValue = getValueByPath<string>(option, displayConfig.fields.value);

                // Handle multi-select vs single-select
                const isAiRecommendation = isMultiSelect
                  ? Array.isArray(aiRecommendedValue) && aiRecommendedValue.includes(optionValue)
            ...
            <div className="space-y-3">
              {!isReadOnly && (
                <div className="font-medium text-sm">
                  ...
                  <span className="text-primary">
                    {
                      (uniqueOptions.find((opt: any) => opt.value === aiRecommendedValue) as any)
                        ?.name
                    }
                  </span>
                </div>
              ...
              <div className="space-y-2">
                {uniqueOptions.map((option: any) => (
                  <div
                    key={option.value}
                    onClick={isReadOnly ? undefined : () => setSelectedValue(option.value)}
                    onKeyDown={
                      isReadOnly
                        ? undefined
                        : (e) => {
                    ...
                    className={cn(
                      ...
                      // Selected state
                      option.value === selectedValue && "border-primary bg-primary/5",
                      // Non-selected: different styles for read-only vs interactive
                      option.value !== selectedValue && isReadOnly && "opacity-60",
                  ...
                  >
                    <div className="font-semibold text-base">
                      ...
                      {option.value === aiRecommendedValue && !isReadOnly && (
                        <span className="ml-2 text-primary text-xs">⭐ AI Recommendation</span>
                      )}
                    </div>
