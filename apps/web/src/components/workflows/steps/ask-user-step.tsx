...
import {
	DirectoryPicker,
	DirectoryPickerHelp,
} from "@/components/ui/directory-picker";
	...
	const handleSubmit = async () => {
		...
		if (config.responseType === "string" && config.validation) {
			if (
				...
				setValidationError(
					`Must be at least ${config.validation.minLength} characters`,
				);
				return;
			}
			if (
				...
				setValidationError(
					`Must be at most ${config.validation.maxLength} characters`,
				);
				return;
			}
		...
		// Submit the value
		onSubmit(
			config.responseType === "number" ? Number.parseFloat(value) : value,
		);
	};
