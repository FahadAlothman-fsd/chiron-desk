...
const AiReasoningContext = React.createContext<AiReasoningContextValue | null>(null);
    ...
    >
      <div className="flex flex-1 items-center gap-2 text-left">
        ...
        {!isStreaming && formattedDuration && (
          <span className="text-[10px] text-muted-foreground">{formattedDuration}</span>
        )}
      </div>
      {children}
      <ChevronDown
...
function AiReasoningContent({ children, className }: AiReasoningContentProps) {
  ...
  return (
    <CollapsiblePrimitive.Content
      ...
      className={cn(
        "border-t overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down",
        className,
      )}
    >
      <div className={cn("p-4 text-xs text-muted-foreground", isStreaming && "animate-pulse")}>
        {children}
        {isStreaming && (
